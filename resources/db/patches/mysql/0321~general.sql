...
-- Create the stored procedure to alter tables
CREATE PROCEDURE alter_tables()
  BEGIN
-- change to myisam if version < 5.6
    select version() regexp '^[0-5]\.[0-5]' into @version;
    IF @version = 1 THEN
      ALTER TABLE bookinfo engine=myisam;
      ALTER TABLE consoleinfo engine=myisam;
      ALTER TABLE musicinfo engine=myisam;
    END IF;
