...
INSERT INTO tv_episodes (videos_id, series, episode, se_complete, title, firstaired, summary)
	(SELECT v.id, CONCAT('S', SUBSTRING_INDEX(tvre.fullep, 'x', 1)), CONCAT('E', SUBSTRING_INDEX(tvre.fullep, 'x', -1)), CONCAT(CONCAT('S', SUBSTRING_INDEX(tvre.fullep, 'x', 1)), CONCAT('E', SUBSTRING_INDEX(tvre.fullep, 'x', -1))), DATE(tvre.airdate), '' FROM videos v INNER JOIN tvrage_episodes tvre USING (rageid));
