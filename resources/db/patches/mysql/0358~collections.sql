...
BEGIN
  ...
  DECLARE cur1 CURSOR FOR SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = databaseName COLLATE utf8_unicode_ci AND TABLE_NAME LIKE "collections\_%";
...
BEGIN
  ...
  DECLARE _table CHAR(255);
  DECLARE _stmt VARCHAR(1000);
  DECLARE cur1 CURSOR FOR SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = databaseName COLLATE utf8_unicode_ci AND TABLE_NAME LIKE "binaries\_%";
  DECLARE CONTINUE HANDLER FOR NOT FOUND SET done = TRUE;
  OPEN cur1;
    myloop: loop FETCH cur1 INTO _table;
      IF done THEN LEAVE myloop; END IF;
