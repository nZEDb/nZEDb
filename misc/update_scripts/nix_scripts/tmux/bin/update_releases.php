...
//$db = new DB();
//echo "Completed adding ".number_format($retcount)." releases in ".$consoletools->convertTime(number_format(microtime(true) - $releases->processReleases, 2)).". ".number_format(array_shift($db->queryOneRow("SELECT COUNT(id) FROM collections WHERE groupid = " . $groupid)))." collections waiting to be created (still incomplete or in queue for creation).\n";
