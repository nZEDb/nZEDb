...
tmux selectp -t 0
tmux splitw -h -p 67 'printf "\033]2;update_binaries\033\\" && bash -c "for i in {1..10000}; do php /var/www/nzedb/misc/update_scripts/update_binaries.php && sleep 1; done"'

tmux selectp -t 0
tmux splitw -v -p 30 'printf "\033]2;nzbcount\033\\"'

tmux selectp -t 2
tmux splitw -v -p 75 'printf "\033]2;backfill\033\\" && bash -c "for i in {1..10000}; do php /var/www/nzedb/misc/update_scripts/backfill.php 20000 && sleep 1; done"'
tmux splitw -v -p 67 'printf "\033]2;backfill\033\\" && php /var/www/nzedb/misc/testing/nzb-import-bulk.php $nzb_path'
tmux splitw -v -p 50 'printf "\033]2;update_releases\033\\" && bash -c "for i in {1..10000}; do php /var/www/nzedb/misc/update_scripts/update_releases.php 1 false && sleep 1; done"'
