...
$mask1 = "\033[1;33m%-16s \033[38;5;214m%-52.52s \n";
$mask2 = "\033[1;33m%-16s \033[38;5;214m%-42.42s \n";
	...
	if ( $running == "TRUE" )
	{
		...
		if ( $seq != 2 )
		{
			...
			if ( $fix_names == "TRUE"  &&  $i == 1 )
			{
				...
				shell_exec("tmux respawnp -t${tmux_session}:1.0 'echo \"\033[38;5;${color}m\"; \
						...
						$_phpn ${DIR}testing/Release_scripts/fixReleaseNames.php 4 true all yes $log; date +\"%D %T\"; $_sleep $fix_timer' 2>&1 1> /dev/null");
			}
			elseif ( $fix_names == "TRUE" )
			{
				...
				shell_exec("tmux respawnp -t${tmux_session}:1.0 'echo \"\033[38;5;${color}m\"; \
						...
						$_phpn ${DIR}testing/Release_scripts/fixReleaseNames.php 3 true all yes $log; date +\"%D %T\"; $_sleep $fix_timer' 2>&1 1> /dev/null");
			}
			else
			{
				$color = get_color($colors_start, $colors_end, $colors_exc);
			...
			if ( $post == 1 && $work_remaining_now > 0 )
			{
				...
				system("tmux respawnp -t${tmux_session}:2.0 'echo \"\033[38;5;${color}m\"; \
						rm -rf $tmpunrar/*; \
			...
			if (( $post_non == "TRUE" ) && (( $movie_releases_proc > 0 ) || ( $tvrage_releases_proc > 0 )))
			{
				...
				system("tmux respawnp -t${tmux_session}:2.1 'echo \"\033[38;5;${color}m\"; \
			...
			if (( $post_amazon == "TRUE" ) && (( $music_releases_proc > 0 ) || ( $book_releases_proc > 0 ) || ( $console_releases_proc > 0 )) && (( $processbooks == 1 ) || ( $processmusic == 1 ) || ( $processgames == 1 )))
			{
				...
				system("tmux respawnp -t${tmux_session}:2.2 'echo \"\033[38;5;${color}m\"; \
		...
		if ( $seq == 1 )
		{
			...
			if (( $import != "0" ) && ( $kill_pp == "FALSE" ))
			{
				...
				system("tmux respawnp -t${tmux_session}:0.1 'echo \"\033[38;5;${color}m\"; \
			...
			if (( $kill_coll == "FALSE" ) && ( $kill_pp == "FALSE" ) && ( TIME() - $time6 <= 4800 ))
			{
				...
				{
					system("tmux respawnp -t${tmux_session}:0.2 'echo \"\033[38;5;${color}m\"; \
			...
			elseif (( $kill_coll == "FALSE" ) && ( $kill_pp == "FALSE" ) && ( TIME() - $time6 >= 4800 ))
			{
				...
				if ( $backfill != "0" )
				{
					system("tmux respawnp -k -t${tmux_session}:0.2 'echo \"\033[38;5;${color}m\"; \
		...
		else
		{
			...
			if (( $backfill == "4" ) && ( $kill_coll == "FALSE" ) && ( $kill_pp == "FALSE" ) && ( TIME() - $time6 <= 4800 ))
			{
				...
				system("tmux respawnp -t${tmux_session}:0.3 'echo \"\033[38;5;${color}m\"; \
			...
			elseif (( $backfill != "0" ) && ( $kill_coll == "FALSE" ) && ( $kill_pp == "FALSE" ) && ( TIME() - $time6 >= 4800 ))
			{
				$color = get_color($colors_start, $colors_end, $colors_exc);
				$log = writelog($panes0[3]);
				system("tmux respawnp -k -t${tmux_session}:0.3 'echo \"\033[38;5;${color}m\"; \
