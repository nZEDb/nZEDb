...
{
	exec("tmux new-window -t $tmux_session -n utils 'printf \"\033]2;fixReleaseNames\033[0m\"'");
	exec("tmux splitw -t $tmux_session:1 -v -p 50 'printf \"\033]2;misc_sorter\033[0m\"'");
	exec("tmux splitw -t $tmux_session:1 -h -p 50 'printf \"\033]2;updateTVandTheaters\033[0m\"'");
	exec("tmux selectp -t 0 && tmux splitw -t $tmux_session:1 -h -p 50 'printf \"\033]2;removeCrapReleases\033[0m\"'");
}

function window_post($tmux_session)
{
	exec("tmux new-window -t $tmux_session -n post 'printf \"\033]2;postprocessing_non_amazon\033[0m\"'");
	exec("tmux splitw -t $tmux_session:2 -v -p 50 'printf \"\033]2;postprocessing_amazon\033[0m\"'");
}

function window_optimize($tmux_session)
{
	exec("tmux new-window -t $tmux_session -n optimize 'printf \"\033]2;update_nZEDb\033[0m\"'");
	exec("tmux splitw -t $tmux_session:3 -v -p 50 'printf \"\033]2;optimize\033[0m\"'");
}
...
if ( $seq == "TRUE" )
{
	exec("cd ${DIR}/update_scripts/nix_scripts/tmux && tmux -f $tmuxconfig new-session -d -s $tmux_session -n Monitor 'printf \"\033]2;Monitor\033[0m\"'");
	exec("tmux selectp -t $tmux_session:0.0 && tmux splitw -t $tmux_session -h -p 67 'printf \"\033]2;update_releases\033[0m\"'");
	exec("tmux selectp -t $tmux_session:0.0 && tmux splitw -t $tmux_session -v -p 33 'printf \"\033]2;nzb-import-bulk\033[0m\"'");
...
{
	exec("cd ${DIR}/update_scripts/nix_scripts/tmux && tmux -f $tmuxconfig new-session -d -s $tmux_session -n Monitor 'echo \"\033]2;Monitor\033[0m\"'");
	exec("tmux selectp -t $tmux_session:0.0 && tmux splitw -t $tmux_session -h -p 67 'printf \"\033]2;update_binaries\033[0m\"'");
	exec("tmux selectp -t $tmux_session:0.0 && tmux splitw -t $tmux_session -v -p 33 'printf \"\033]2;nzb-import\033[0m\"'");
	exec("tmux selectp -t $tmux_session:0.2 && tmux splitw -t $tmux_session -v -p 67 'printf \"\033]2;backfill\033[0m\"'");
	exec("tmux splitw -t $tmux_session -v -p 50 'printf \"\033]2;update_releases\033[0m\"'");
