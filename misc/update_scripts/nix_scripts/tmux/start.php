...
// exec should always return an array for the results into the $nntpkill variable.  Zero length means no output, no match for grep.
if ( count($nntpkill) === 0  ) { echo $c->info("No NNTPProxy tmux session to kill"); }
	else {
        	echo $c->primary("Found NNTPProxy tmux session and killing it");
...
if ($session != 0)
	...
	echo $c->notice("The above is just a notice generated by tmux. \nWhen starting this script, we first check to that you do not have a tmux session currently running. If you do not, the above notice is generated by tmux.\It is not an error.");
//passthru("clear");
