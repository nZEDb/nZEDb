...
cur.execute("SELECT r.id, r.name, g.name AS groupname FROM releases r LEFT JOIN groups g ON r.groupid = g.id WHERE ( relnamestatus in (0, 1, 20, 21, 22) OR categoryid BETWEEN 7000 and 7999 ) AND nzbstatus = 1 AND reqidstatus in (0, -1) AND ( r.name REGEXP '^\\[[[:digit:]]+\\]' = 1 OR r.searchname REGEXP '^\\[[[:digit:]]+\\]' = 1 ) limit 1000")
datas = cur.fetchall()
