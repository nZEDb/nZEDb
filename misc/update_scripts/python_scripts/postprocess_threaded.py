...
if len(sys.argv) > 1 and sys.argv[1] == "additional":
	...
	cur.execute("SELECT (SELECT value FROM site WHERE setting = 'postthreads') AS a, (SELECT value FROM site WHERE setting = 'maxaddprocessed') AS b, (SELECT value FROM site WHERE setting = 'maxnfoprocessed') AS c, (SELECT value FROM site WHERE setting = 'maximdbprocessed') AS d, (SELECT value FROM site WHERE setting = 'maxrageprocessed') AS e, (SELECT value FROM site WHERE setting = 'maxsizetopostprocess') AS f, (SELECT value FROM site WHERE setting = 'tmpunrarpath') AS g, (SELECT value FROM tmux WHERE setting = 'post') AS h, (SELECT value FROM tmux WHERE setting = 'post_non') AS i, (SELECT count(*) FROM releases WHERE nfostatus = -1) as j, (SELECT count(*) FROM releases WHERE nfostatus = -2) as k, (SELECT count(*) FROM releases WHERE nfostatus = -3) as l, (SELECT count(*) FROM releases WHERE nfostatus = -4) as m, (SELECT count(*) FROM releases WHERE nfostatus = -5) as n, (SELECT count(*) FROM releases WHERE nfostatus = -6) as o")
	dbgrab = cur.fetchall()
	ps1 = format(int(dbgrab[0][9]))
	ps2 = format(int(dbgrab[0][10]))
	ps3 = format(int(dbgrab[0][11]))
...
if sys.argv[1] == "additional":
	...
	if len(datas) < process_nfo:
		...
		run = "SELECT searchname, id FROM releases WHERE (bitwise & 260) = 260 AND searchname IS NOT NULL AND rageid = -1 AND categoryid IN ( SELECT id FROM category WHERE parentid = 5000 ) ORDER BY postdate DESC LIMIT %s"
