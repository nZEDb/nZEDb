...
$mask = "\033[1;33m%-50.50s %22.22s %22.22s %22.22s %22.22s %22.22s %22.22s %22.22s\n";
...
if ($rels = $db->queryDirect(sprintf("SELECT name, backfill_target, first_record_postdate, last_updated,
										...
										LEFT OUTER JOIN ( SELECT groupid, COUNT(id) AS num FROM releases WHERE relnamestatus not in (0, 1) GROUP BY groupid ) ren ON ren.groupid = groups.id
