...
{
    if ($rels = $db->query(sprintf("select name, backfill_target, first_record_postdate, last_updated from groups where active = 1 and backfill = 1 order by first_record_postdate DESC %s", $limit)))
    {
        foreach ($rels as $rel)
        {
...
{
	if ($rels = $db->query(sprintf("select name, backfill_target, first_record_postdate, last_updated from groups where active = 1 and backfill = 1 order by first_record_postdate ASC %s", $limit)))
	{
		foreach ($rels as $rel)
		{
			printf($mask, $rel['name'], $rel['backfill_target'], $rel['first_record_postdate'], $rel['last_updated']);
