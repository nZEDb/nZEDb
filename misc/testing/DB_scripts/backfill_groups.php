...
{
    if ($rels = $db->query(sprintf("select name, backfill_target, first_record_postdate, last_updated, last_updated, last_record-first_record as 'headers downloaded' from groups where backfill = 1 order by first_record_postdate DESC %s", $limit)))
    {
        foreach ($rels as $rel)
        {
...
{
	if ($rels = $db->query(sprintf("select name, backfill_target, first_record_postdate, last_updated, last_updated, last_record-first_record as 'headers downloaded' from groups where backfill = 1 order by first_record_postdate ASC %s", $limit)))
	{
		foreach ($rels as $rel)
		{
			printf($mask, $rel['name'], $rel['backfill_target'], $rel['first_record_postdate'], $rel['last_updated'], $rel['headers downloaded']);
