...
if ($argv[1] < $predbv["v"])
	...
	$filenums = $argv[1];
else if ($argv[1] == 0 || $argv[1] > 1419)
	exit("Wrong argument. It must be a number between 1 and 1419.\n");

$done = 0;
foreach (range($predbv["v"], $filenums) as $filenumber)
{
	$filenump = str_pad($filenumber, 4, '0', STR_PAD_LEFT);
