...
class AniDBstandAlone
{
	...
	public function getAniDBInfo($exitcount)
	{
		...
		// remove anything that has not been updated in 60 days, as we can assume this is too old to be valid or in progress after 7
		foreach($animetitles as $value) {
			...
			if($AniDBAPIArrayOld == False) {
				...
				if($anidbstartdate == 0 && $anidbenddate == 0) {
					...
					// in this case anime may not have an end date and should be considered in progress as the end date is just unknown
					...
					($AniDBAPIArrayOld['AnimeInProgress'] == True && (time() - $AniDBAPIArrayOld['unixtime']) > 604800) )
				{
					...
					// this means in the next section we only need to deal with new entries
...
/* Holding on to this in case we want it again as it has some uses, but currently we manage this in the first foreach statement, so there is no need for it any longer
			else
			{
				if ($this->echooutput)
