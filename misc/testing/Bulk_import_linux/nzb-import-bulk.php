...
else
{
	...
	foreach($objects as $filestoprocess => $nzbFile){
		...
		if (!$importfailed)
		{
			$relguid = sha1(uniqid());
			$nzb = new NZB();
			$groupName = $db->queryOneRow(sprintf("SELECT name FROM groups WHERE ID = %d", $groupID));
			$cleanerArray = $namecleaning->collectionsCleaner($subject, $groupName["name"]);
			if ($cleanerArray !== false)
				$cleanerName = $cleanerArray["clean"];
			else
				$cleanerName = $subject;
