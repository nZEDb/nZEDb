	...
	if ($total > 0)
	{
		...
		foreach ($res as $row)
		{
		   if($cleanerName = trim(releaseCleaner($row['name'], $row['groupid'], $row['id'])))
			...
			//elseif (preg_match('/^\[?\d*\].+?yEnc/i', $row['name']))
				//echo $row['name']."\n";
...
function releaseCleaner($subject, $groupid, $id)
{
	...
	if ($groupName === "alt.binaries.classic.tv.shows")
	{
		...
		elseif (preg_match('/^(?P<title>Yancy.+) \[\d+ of \d+\].+?("|#34;).+("|#34;) yEnc/i', $subject, $match))
		{
		...
		elseif (preg_match('/^("|#34;)(?P<title>Yancy.+?)\.(par|zip|rar|nfo|txt).+?("|#34;) yEnc/i', $subject, $match))
		{
			$cleanerName = $match['title'];
			if (!empty($cleanerName))
				return $cleanerName;
		...
		elseif (preg_match('/\((?P<title>.+)\) \[\d+\/\d+\][ _-]{0,3}("|#34;).+("|#34;) yEnc/i', $subject, $match))
		{
			$cleanerName = $match['title'];
			if (!empty($cleanerName))
				return $cleanerName;
	...
	elseif (preg_match('/^\[\d+\][ _-]{0,3}(.+?)[ _-]{0,3}(\[(reup|full|repost.+?|part|re-repost|xtr|sample)\])?[ _-]{0,3}\[ ?(?P<title>.+) ?\][ _-]{0,3}.+?[ _-]{0,3}([\[\(]\d+[~\/]\d+[\)\]]|\[\d+\])[ _-]{0,3}("|#34;)?.+("|#34;)?[ _-]{0,3}yEnc$/i', $subject, $match))
	{
		$cleanerName = $match['title'];
		if (!empty($cleanerName))
			return $cleanerName;
	...
	elseif (preg_match('/^\[\d+\][ _-]{0,3}\[.+\][ _-]{0,3}(\[(reup|full|repost.+?|part|re-repost|xtr|sample)\])[ _-]{0,3}\[(?P<title>.+)\][ _-]{0,3}\[\d+[\/-]\d+\][ _-]{0,3}.+yEnc/i', $subject, $match))
	{
		$cleanerName = $match['title'];
		if (!empty($cleanerName))
			return $cleanerName;
	...
	elseif (preg_match('/^\[\d+\][ _-]{0,3}#(Movie|TV)gen[ _-]{0,3}(?P<title>.+)[ _-]{0,3}\[\d+\/\d+\][ _-]{0,3}("|#34;).+("|#34;) yEnc/i', $subject, $match))
	{
		$cleanerName = $match['title'];
		if (!empty($cleanerName))
			return $cleanerName;
	...
	elseif (preg_match('/^\[\d+\][ _-]{0,3}(?P<title>.+)( \[\d+\/\d+\] )?[ _-]{0,3}("|#34;).+?(m3u|jpg|mp3|par2|nfo|nzb|rar|zip)("|#34;)[ _-]{0,3}yEnc/i', $subject, $match))
	{
		$cleanerName = $match['title'];
		if (!empty($cleanerName))
			return $cleanerName;
	...
	elseif (preg_match('/^\[\d+\][ _-]{0,3}\(#a.b.g.x@EFnet\)[ _-]{0,3}(?P<title>.+) - .+?yEnc/i', $subject, $match))
	{
		$cleanerName = $match['title'];
		if (!empty($cleanerName))
			return $cleanerName;
	...
	elseif (preg_match('/^\[\d*\][ _-]{0,3}("|#34;)(?P<title>.+)\.(rar|par2|zip)("|#34;) yEnc/i', $subject, $match))
	{
		$cleanerName = $match['title'];
		if (!empty($cleanerName))
			return $cleanerName;
	...
	elseif (preg_match('/^[ <\[]{0,2}TOWN[ >\]]{0,2}[ _-]{0,3}[ <\[]{0,2}www\.town\.ag[ >\]]{0,2}[ _-]{0,3}[ <\[]{0,2}partner of www.ssl-news\.info[ >\]]{0,2}[ _-]{0,3}\[\d+\/\d+\][ _-]{0,3}("|#34;)(?P<title>.+)\.(par|vol|rar|nfo).*?("|#34;).+?yEnc$/i', $subject, $match))
	{
		$cleanerName = $match['title'];
		if (!empty($cleanerName))
			return $cleanerName;
	...
	elseif (preg_match('/^<kere\.ws>[ _-]{0,3}(TV|Filme)[ _-]{0,3}\d+[ _-]{0,3}(?P<title>.+)[ _-]{0,3}\[\d+\/\d+\][ _-]{0,3}("|#34;).+?("|#34;) yEnc$/i', $subject, $match))
	{
		$cleanerName = $match['title'];
		if (!empty($cleanerName))
			return $cleanerName;
	...
	elseif (preg_match('/^<TOWN><www\.town\.ag > <partner of www\.ssl-news\.info > (?P<title>.+) \[\d+\/\d+\][ _-]{0,3}("|#34;).+?("|#34;).+?yEnc$/i', $subject, $match))
	{
		$cleanerName = $match['title'];
		if (!empty($cleanerName))
			return $cleanerName;
	...
	elseif (preg_match('/^\(www\.Thunder-News\.org\) ?>(?P<title>.+)< ?<Sponsored.+?>[ _-]{0,3}(\(\d+\/\d+\)|\[\d+\/\d+\])[ _-]{0,3}("|#34;).+?("|#34;) yEnc$/i', $subject, $match))
	{
		$cleanerName = $match['title'];
		if (!empty($cleanerName))
			return $cleanerName;
	...
	elseif (preg_match('/^>ghost-of-usenet\.org<< ?(?P<title>.+) ?>>www.+>[ _-]{0,3}("|#34;)?.+("|#34;)? ?yEnc$/i', $subject, $match))
	{
		$cleanerName = $match['title'];
		if (!empty($cleanerName))
			return $cleanerName;
	...
	elseif (preg_match('/^\[ nEwZ\[NZB\]\.iNFO \][ _-]{0,3}\[ (?P<title>.+) \][ _-]{0,3}File \[\d+\/\d+\]: ("|#34;).+("|#34;) yEnc$/i', $subject, $match))
	{
		$cleanerName = $match['title'];
		if (!empty($cleanerName))
			return $cleanerName;
	...
	elseif (preg_match('/^(?P<title>.+Novarip) \[\d+\/\d+\][ _-]{0,3}("|#34;).+?("|#34;) yEnc$/i', $subject, $match))
	{
		$cleanerName = $match['title'];
		if (!empty($cleanerName))
			return $cleanerName;
	...
	elseif (preg_match('/^\(\d+\/\d+\)[ _-]{0,3}\[Lords-of-Usenet\] (<<|\(\()(Partner|Sponsor).+(>>|\)\)) presents (?P<title>.+)[ _-]{0,3}("|#34;).+("|#34;).+yEnc$/i', $subject, $match))
	{
		$cleanerName = $match['title'];
		if (!empty($cleanerName))
			return $cleanerName;
	...
	elseif (preg_match('/^\(\?+\) \[\d+\/\d+\][ _-]{0,3}("|#34;)(?P<title>.+)("|#34;) yEnc$/i', $subject, $match))
	{
		$cleanerName = $match['title'];
		if (!empty($cleanerName))
			return $cleanerName;
	...
	elseif (preg_match('/^\(www\.Thunder-News\.org\) ?>(?P<title>.+)< <Sponsored.+?>[ -]{0,7}("|#34;).+("|#34;) yEnc$/i', $subject, $match))
	{
		$cleanerName = $match['title'];
		if (!empty($cleanerName))
			return $cleanerName;
	...
	elseif (preg_match('/^\[foreign\][ _-]{0,3}\[ (?P<title>.+) \][ -]?\[\d+\/\d+\][ _-]{0,3}("|#34;).+("|#34;) yEnc$/i', $subject, $match))
	{
		$cleanerName = $match['title'];
		if (!empty($cleanerName))
			return $cleanerName;
	...
	elseif (preg_match('/^\(www\.Thunder-News\.org\) ?\)(?P<title>.+)\( \(Sponsored.+\)[ _-]{0,3}\(\d+\/\d+\)[ _-]{0,3}("|#34;).+("|#34;) yEnc$/i', $subject, $match))
	{
		$cleanerName = $match['title'];
		if (!empty($cleanerName))
			return $cleanerName;
	...
	elseif (preg_match('/^<<<usenet-space-cowboys\.info>>>.+>< ("|#34;)(?P<title>.+)_usenet-space-cowbo?ys.+("|#34;) >< \d+\/\d+ \(.+\) ><.+> yEnc$/i', $subject, $match))
	{
		$cleanerName = $match['title'];
		if (!empty($cleanerName))
			return $cleanerName;
	...
	elseif (preg_match('/^<kere\.ws\/illuminatenboard\.org>[ _-]{0,3}ID[ _-]{0,3}\d+[ _-]{0,3}(?P<title>.+) \[\d+\/\d+\][ _-]{0,3}("|#34;).+("|#34;) yEnc$/i', $subject, $match))
	{
		$cleanerName = $match['title'];
		if (!empty($cleanerName))
			return $cleanerName;
	...
	elseif (preg_match('/^(?P<title>.+)\[.+usenet-4all\.info\]-\[.+\]-\(\d+\/\d+\) ("|#34;).+("|#34;) yEnc$/i', $subject, $match))
	{
		$cleanerName = $match['title'];
		if (!empty($cleanerName))
			return $cleanerName;
	...
	elseif (preg_match('/^\[Lords-of-Usenet\.org\][ _-]{0,3}<Sponsored.+>[ _-]{0,3}proudly.+_Proudly_presents_(?P<title>.+)[ _-]{0,3}\[\d+\/\d+\][ _-]{0,3}("|#34;).+("|#34;) yEnc$/i', $subject, $match))
	{
		$cleanerName = $match[1];
		if (!empty($cleanerName))
			return $cleanerName;
	...
	elseif (preg_match('/^\[Lords-of-Usenet\.org\][ _]<Sponsored.+> proudly presents:(?P<title>.+) \[\d+\/\d+\][ _-]{0,3}("|#34;).+("|#34;) yEnc$/i', $subject, $match))
	{
		$cleanerName = $match['title'];
		if (!empty($cleanerName))
			return $cleanerName;
	...
	elseif (preg_match('/^\[:sek9:\]\[[-\w]+\][ _-]{0,3}\[:(?P<title>.+):\][ _-]{0,3}\[\d+\/\d+\][ _-]{0,3}("|#34;).+("|#34;) yEnc$/i', $subject, $match))
	{
		$cleanerName = $match['title'];
		if (!empty($cleanerName))
			return $cleanerName;
	...
	elseif (preg_match('/^(?P<title>.+)\[.+?usenet-4all\.info\][ _-]{0,3}\[.+\][ _-]{0,3}\(\d+\/\d+\) ("|#34;).+("|#34;) yEnc$/i', $subject, $match))
	{
		$cleanerName = $match['title'];
		if (!empty($cleanerName))
			return $cleanerName;
	...
	elseif (preg_match('/^<<<(Thor2204|Thor)>>><<<(?P<title>.+)>>>usenet-space-cowboys.+<<<Powered.+>< ("|#34;).+("|#34;).+> yEnc$/i', $subject, $match))
	{
		$cleanerName = $match['title'];
		if (!empty($cleanerName))
			return $cleanerName;
	...
	elseif (preg_match('/^<<<Nimue>>><<<(?P<title>.+)>>>.+<<<Powered by.+yEnc$/i', $subject, $match))
	{
		$cleanerName = $match['title'];
		if (!empty($cleanerName))
			return $cleanerName;
	...
	elseif (preg_match('/^\[(?P<title>.+)\][ _-]{0,3}\[.+usenet-4all\.info][ _-]{0,3}\[.+\][ _-]{0,3}\[powered.+\] \(\d+\/\d+\) ("|#34;).+("|#34;) yEnc$/i', $subject, $match))
	{
		$cleanerName = $match['title'];
		if (!empty($cleanerName))
			return $cleanerName;
	...
	elseif (preg_match('/^<<<.+?>>>CowBoyUp26<<<Powered by.+>< ("|#34;)(?P<title>.+)_usenet-space-cowboys.+("|#34;) >< \d+\/\d+ \(.+?\) ><.+> yEnc$/i', $subject, $match))
	{
		$cleanerName = $match['title'];
		if (!empty($cleanerName))
			return $cleanerName;
	...
	elseif (preg_match('/^<kere\.ws> (?P<title>.+)[ _-]{0,3}\[\d+\/\d+\][ _-]{0,3}("|#34;).+("|#34;) yEnc$/i', $subject, $match))
	{
		$cleanerName = $match['title'];
		if (!empty($cleanerName))
			return $cleanerName;
	...
	elseif (preg_match('/^(\(snake\)|panter|wildrose|shadowman|P2H)[ _-]{0,3}(\[\d+\/\d+\])?[ _-]{0,3}("|#34;)(info-|P2H-)?(?P<title>.+)( \.){0,2}(part\d+\.rar|vol\d+\+\d+\.par2|rar\.vol\d+\+\d+\.PAR2|par2|rar|rar\.par2|mkv\.par2|(avi|dvd5|mkv)\.part\d+\.rar|nfo|zip|nzb)("|#34;) yEnc$/i', $subject, $match))
	{
		$cleanerName = $match['title'];
		if (!empty($cleanerName))
			return $cleanerName;
	...
	elseif (preg_match('/^\[www\.allyourbasearebelongtous\.pw\][ _-]{0,3}\[(?P<title>.+)\][ _-]{0,3}("|#34;).+("|#34;)[ _-]{0,3}.+yEnc$/i', $subject, $match))
	{
		$cleanerName = $match['title'];
		if (!empty($cleanerName))
			return $cleanerName;
	...
	elseif (preg_match('/^\([ _-]{0,3}(?P<title>.+)[ _-]{0,3}\)[ _-]{0,3}\)ghost-of-usenet\.org\([ _-]{0,3}\(\d+\/\d+\) ?\).+("|#34;).+("|#34;)[ _-]{0,3}yEnc$/i', $subject, $match))
	{
		$cleanerName = $match['title'];
		if (!empty($cleanerName))
			return $cleanerName;
	...
	elseif (preg_match('/^\(Public\)[ _-]{0,3}\(FULL\)[ _-]{0,3}\(.+efnet\)[ _-]{0,3}\[\d+\/\d+\][ _-]{0,3}\(\?+\)[ _-]{0,3}\[\d+\/\d+\][ _-]{0,3}("|#34;)(?P<title>.+)[ _-]{0,3}(\(1\))?( |\.| \.)?(part\d+\.rar|vol\d+\+\d+\.par2|rar\.vol\d+\+\d+\.PAR2|par2|rar|rar\.par2|mkv\.par2|(avi|dvd5|mkv)\.part\d+\.rar|nfo|zip|nzb)("|#34;) yEnc/i', $subject, $match))
	{
		$cleanerName = $match['title'];
		if (!empty($cleanerName))
			return $cleanerName;
	...
	elseif (preg_match('/^P2H[ _-]{0,3}(?P<title>.+)[ _-]{0,3}("|#34;).+("|#34;) yEnc$/i', $subject, $match))
	{
		$cleanerName = $match['title'];
		if (!empty($cleanerName))
			return $cleanerName;
	...
	elseif (preg_match('/^<kere\.ws> \[[ _-]{0,3}(?P<title>.+)[ _-]{0,3}\][ _-]{0,3}\[\d+\/\d+\][ _-]{0,3}("|#34;).+("|#34;) yEnc$/i', $subject, $match))
	{
		$cleanerName = $match['title'];
		if (!empty($cleanerName))
			return $cleanerName;
	...
	elseif (preg_match('/\(\d+\/\d+\)[ _-]{0,3}("|#34;).+("|#34;).+\[Foreign\] (?P<title>.+) yEnc$/i', $subject, $match))
	{
		$cleanerName = $match['title'];
		if (!empty($cleanerName))
			return $cleanerName;
	...
	elseif (preg_match('/^\[foreign\][ _-]{0,3}\[ (?P<title>.+) \][ _-]{0,3}("|#34;).+("|#34;)/i', $subject, $match))
	{
		$cleanerName = $match['title'];
		if (!empty($cleanerName))
			return $cleanerName;
	...
	elseif (preg_match('/^\[Lords-of-Usenet\.org\][ _-]{0,3}\[Partner.+\]\(\d+\/\d+\)[ _-]{0,3}("|#34;)(?P<title>.+)("|#34;) yEnc$/i', $subject, $match))
	{
		$cleanerName = $match['title'];
		if (!empty($cleanerName))
			return $cleanerName;
	...
	elseif (preg_match('/^(?P<title>.+)\[.+?usenet-4all\.info\]-\[.+\][ _-]{0,3}("|#34;).+("|#34;) yEnc$/i', $subject, $match))
	{
		$cleanerName = $match['title'];
		if (!empty($cleanerName))
			return $cleanerName;
	...
	elseif (preg_match('/^\(\d+\/\d+\) ("|#34;)(?P<title>.+) www\.brothers-of-usenet\.org.+("|#34;) yEnc$/i', $subject, $match))
	{
		$cleanerName = $match['title'];
		if (!empty($cleanerName))
			return $cleanerName;
	...
	elseif (preg_match('/^\[Lords-of-Usenet\][ _-]{0,3}(<<|\(\(|\[)(Partner|Sponsor).+(>>|\)\)|\])\(\d+\/\d+\)[ _-]{0,3}("|#34;)(?P<title>.+)("|#34;) yEnc$/i', $subject, $match))
	{
		$cleanerName = $match['title'];
		if (!empty($cleanerName))
			return $cleanerName;
	...
	//94 out of 3657512
	elseif (preg_match('/^\[Lords-of-Usenet\.org\][ _-]{0,3}<?<Sponsored.+>>?[ _-]{0,3}(?P<title>.+)[ _-]{0,3}\(\d+\/\d+\)[ _-]{0,3}("|#34;).+("|#34;)[ _-]{0,3}.+yEnc$/i', $subject, $match))
	{
		$cleanerName = $match['title'];
		if (!empty($cleanerName))
			return $cleanerName;
	...
	//602 out of 3657418
	elseif (preg_match('/\[www\.Lords-of-Usenet\.org\][ _-]{0,3}\[.+\]\(\d+\/\d+\) ("|#34;)(?P<title>.+)("|#34;) yEnc$/i', $subject, $match))
	{
		$cleanerName = $match['title'];
		if (!empty($cleanerName))
			return $cleanerName;
	...
	//120 out of 3656816
	elseif (preg_match('/^\[Lords-of-Usenet\.org\][ _-]{0,3}<?<Sponsored.+>>?_(?P<title>.+) \[\d+\/\d+\][ _-]{0,3}("|#34;).+("|#34;) yEnc$/i', $subject, $match))
	{
		$cleanerName = $match['title'];
		if (!empty($cleanerName))
			return $cleanerName;
	...
	//531 out of 3652904
	elseif (preg_match('/^Brothers-of-Usenet\.org.+("|#34;)(?P<title>.+)("|#34;)\[\d+\/\d+\][ _-]{0,3}("|#34;).+("|#34;) yEnc$/i', $subject, $match))
	{
		$cleanerName = $match['title'];
		if (!empty($cleanerName))
			return $cleanerName;
	...
	//568 out of 5398477
	elseif (preg_match('/^<<<MetalDept>>><<<(?P<title>.+)>>>.+<<< ("|#34;).+("|#34;)>\[\d+\/\d+\].+yEnc$/i', $subject, $match))
	{
		$cleanerName = $match['title'];
		if (!empty($cleanerName))
			return $cleanerName;
	...
	//238 out of 5398477
	elseif (preg_match('/^<<<MetalDept>>><<<(?P<title>.+)>>>usenet-of-outlaws.info.+<<< ("|#34;).+("|#34;)>\[\d+\/\d+\].+/i', $subject, $match))
	{
		$cleanerName = $match['title'];
		if (!empty($cleanerName))
			return $cleanerName;
	...
	//127 out of 3652373
	elseif (preg_match('/^Brothers-of-Usenet\.org \(\d+\/\d+\)[ _-]{0,3}("|#34;)(?P<title>.+)\.(par|rar|nfo|vol).+("|#34;)[ _-]{0,3}.+yEnc$/i', $subject, $match))
	{
		$cleanerName = $match['title'];
		if (!empty($cleanerName))
			return $cleanerName;
	...
	//108 out of 3583346
	elseif (preg_match('/^\[NZBMatrix\.com\][ _-]{0,3}\[ (?P<title>.+) \] \[\d+\/\d+\][ _-]{0,3}("|#34;).+("|#34;) yEnc$/i', $subject, $match))
	{
		$cleanerName = $match['title'];
		if (!empty($cleanerName))
			return $cleanerName;
	...
	//1 out of 3583238
	elseif (preg_match('/^\[ VintageReduction \][ _-]{0,3}\[ (?P<title>.+) \][ _-]{0,3}\[\d+\/\d+\][ _-]{0,3}.+yEnc$/i', $subject, $match))
	{
		$cleanerName = $match['title'];
		if (!empty($cleanerName))
			return $cleanerName;
	...
	//1798 out of 3681602
	elseif (preg_match('/^>ghost-of-usenet\.org< ?(?P<title>.+)>Sponsored.+< ?\(\d+\/\d+\)[ _-]{0,3}("|#34;)?.+("|#34;)?[ _-]{0,3}yEnc$/i', $subject, $match))
	{
		$cleanerName = $match['title'];
		if (!empty($cleanerName))
			return $cleanerName;
	...
	//862 out of 3679804
	elseif (preg_match('/^<[ _-]{0,3}(?P<title>.+)\.by\.GhostUp10[ _-]{0,3}> ?>ghost-of-usenet\.org<[ _-]{0,3}\(\d+\/\d+\)[ _-]{0,3}>www.+<[ _-]{0,3}("|#34;)?.+("|#34;)?[ _-]{0,3}yEnc$/i', $subject, $match))
	{
		$cleanerName = $match['title'];
		if (!empty($cleanerName))
			return $cleanerName;
	...
	//
	elseif (preg_match('/^\(\(\(CowboyUp2012\)\)\)[ _-]{0,3}\(\(\((?P<title>.+)\)\)\)[ _-]{0,3}.+yEnc$/i', $subject, $match))
	{
		$cleanerName = $match['title'];
		if (!empty($cleanerName))
			return $cleanerName;
	...
	elseif (preg_match('/^\[ TOWN \][ _-]{0,3}\[ www\.town\.ag \][ _-]{0,3}\[ (?P<title>.+) \][ _-]{0,3}\[\d+\/\d+\][ _-]{0,3}("|#34;).+\.(par|vol|rar|nfo).*("|#34;) yEnc$/i', $subject, $match))
	{
		$cleanerName = $match['title'];
		if (!empty($cleanerName))
			return $cleanerName;
	...
	elseif (preg_match('/^\[ TOWN \][ _-]{0,3}\[ www\.town\.ag \][ _-]{0,3}\[ partner of www\.ssl-news\.info \][ _-]{0,3}\[ .* \] \[\d+\/\d+\][ _-]{0,3}("|#34;)(?P<title>.+)(\.part\d+)?(\.(par2|(vol.+?))"|\.[a-z0-9]{3}"|")[ _-]{0,3}/i', $subject, $match))
	{
		$cleanerName = $match['title'];
		if (!empty($cleanerName))
			return $cleanerName;
	...
	elseif (preg_match('/^<TOWN><www.town.ag >[ _-]{0,3}(?P<title>.+)[ _-]{0,3}<partner of www\.ssl-news\.info >[ _-]{0,3}\[\d+\/\d+\][ _-]{0,3}("|#34;).+?("|#34;) yEnc$/i', $subject, $match))
	{
		$cleanerName = $match['title'];
		if (!empty($cleanerName))
			return $cleanerName;
	...
	elseif (preg_match('/^\[?(?P<title>.+)\]?[ _-]{0,3}\[ZED for usenet-4all.info\][ _-]{0,3}\[.+?\][ _-]{0,3}\(\d+\/\d+\)[ _-]{0,3}("|#34;).+?("|#34;) yEnc$/i', $subject, $match))
	{
		$cleanerName = $match['title'];
		if (!empty($cleanerName))
			return $cleanerName;
	...
	elseif (preg_match('/^3000 Fiction Ebooks Collection[ _-]{0,3}("|#34;)(?P<title>.+)\.(txt|pdf|lit|doc|rtf|chm|par2)("|#34;) yEnc$/i', $subject, $match))
	{
		$cleanerName = $match['title'];
		if (!empty($cleanerName))
			return $cleanerName;
	...
	elseif (preg_match('/^\[united-forums.co.uk\].+?\[\d+\/\d+\][ _-]{0,3}("|#34;)?(?P<title>.+)( -WWW.UNITED-FORUMS.CO.UK)?(\.|-|_)+(rar|zip|7z)("|#34;)? yEnc$/i', $subject, $match))
	{
		$cleanerName = $match['title'];
		if (!empty($cleanerName))
			return $cleanerName;
	...
	elseif (preg_match('/^brothers-of-usenet\.net\(\)(?P<title>.+)\(\)\(\d+\/\d+\) ("|#34).+("|#34).+?[ _-]{0,3}.+?[ _-]{0,3}yEnc$/i', $subject, $match))
	{
		$cleanerName = $match['title'];
		if (!empty($cleanerName))
			return $cleanerName;
	...
	elseif (preg_match('/^New eBooks.+[ _-]{0,3}("|#34;)(?P<title>.+)\.(par|vol|rar|nfo).*?("|#34;)/i', $subject, $match))
	{
		$cleanerName = $match['title'];
		if (!empty($cleanerName))
			return $cleanerName;
	...
	elseif (preg_match('/^\[ ?nEwZ\[NZB\]\.iNFO ?\][ _-]{0,3}\[ ?(?P<title>.*? ?)\][ _-]{0,3}.*?\[\d+\/\d+\][ -:]{0,3}("|#34;).+("|#34;) yEnc$/i', $subject, $match))
	{
		$cleanerName = $match['title'];
		if (!empty($cleanerName))
			return $cleanerName;
	...
	elseif (preg_match('/^\[NEW DOX\][ _-]{0,3}(?P<title>.+)[ _-]{0,3}\[\d+\/\d+\][ _-]{0,3}("|#34;).+("|#34;)/i', $subject, $match))
	{
		$cleanerName = $match['title'];
		if (!empty($cleanerName))
			return $cleanerName;
	...
	elseif (preg_match('/^http:\/\/nzbroyalty\.com[ _-]{0,3}UK Top 40 (Solo Artist|Compilation) Charts .+[ _-]{0,3}\[\d+\/\d+\][ _-]{0,3}("|#34;)\d+[ _-]{0,3}(?P<title>.+)\.(par|vol|rar|nfo).*?("|#34;)	/i', $subject, $match))
	{
		$cleanerName = $match['title'];
		if (!empty($cleanerName))
			return $cleanerName;
	...
	elseif (preg_match('/^http:\/\/nzbroyalty\.com[ _-]{0,3}UK Top 40 (Solo Artist|Compilation) Charts .+[ _-]{0,3}\[\d+\/\d+\][ _-]{0,3}("|#34;)(?P<title>.+)\.(par|vol|rar|nfo).*?("|#34;)/i', $subject, $match))
	{
		$cleanerName = $match['title'];
		if (!empty($cleanerName))
			return $cleanerName;
	...
	elseif (preg_match('/^\Dutch: Updates & NIEUW maand.+ \[NMS\] \[\d+\/\d+\][ _-]{0,3} ("|#34;)(?P<title>.+)("|#34;)/i', $subject, $match))
	{
		$cleanerName = $match['title'];
		if (!empty($cleanerName))
			return $cleanerName;
