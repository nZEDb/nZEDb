...
foreach (range($lppo, $wp) as $number)
{
	...
	if ($number == 1)
		$db->query("select unix_timestamp(adddate),title,category,replace(size,'[a-zA-Z]',''),predate from predb where adddate > '".$lpdpo."' limit 1,10000 INTO OUTFILE 'data001.txt' FIELDS TERMINATED BY ',' ENCLOSED BY '~' LINES TERMINATED BY '\n'");
	else
		$db->query("select unix_timestamp(adddate),title,category,replace(size,'[a-zA-Z]',''),predate from predb where adddate > '".$lpdpo."' limit ${loop}, 10000 INTO OUTFILE 'data${number2}.txt' FIELDS TERMINATED BY ',' ENCLOSED BY '~' LINES TERMINATED BY '\n'");
