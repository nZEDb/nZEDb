...
if (isset($argv[1]) && $argv[1] == 'export' && isset($argv[2])) {
	...
	$db->queryDirect('INSERT INTO crap (title, nfo, size, category, predate, adddate, source, md5, requestid, groupid) SELECT t.title, t.nfo, t.size, t.category, t.predate, t.adddate, t.source, t.md5, t.requestid, IF(g.id IS NOT NULL, g.id, 0) FROM tmp_pre t LEFT OUTER JOIN groups g ON t.groupname = g.name ON DUPLICATE KEY UPDATE crap.nfo = IF(crap.nfo is null, t.nfo, crap.nfo), crap.size = IF(crap.size is null, t.size, crap.size), crap.category = IF(crap.category is null, t.category, crap.category), crap.requestid = IF(crap.requestid = 0, t.requestid, crap.requestid), crap.groupid = IF(g.id IS NOT NULL, g.id, 0)');
