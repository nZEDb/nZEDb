		...
		if (isset($output[0]) && strlen($output[0]) > 8) {
			foreach ($output as $line) {
				...
				if ($time[1] >= $killtime) {
					...
					// kill pid
					echo $pdo->cli->alternate("PID: $line1[0] USER: $line1[1] TIME: $time[0] CMD: $line");
					usleep(10000);
					exec("kill " . $line1[0] . " 2>&1 1> /dev/null");
					// reset good timer
					$time1 = TIME();
				} else {
					echo $pdo->cli->primary("PID: $line1[0] USER: $line1[1] TIME: $time[0] CMD: $line");
				}
			}
		} else {
			echo $pdo->cli->header("update_binaries or backfill does not appear to be running");
			$time1 = TIME();
		}

		echo $pdo->cli->header("Monitoring ${threads} threads.");
