...
echo $pdo->cli->warning("This script resets all table id columns.");
...
if (count($list) == 0) {
	...
	foreach ($list as $column) {
		if (strtolower($column['column_name']) === 'id' && strtolower($column['extra']) === 'auto_increment') {
			...
			echo $pdo->cli->primary("ALTER TABLE " . $column['table_name'] . " AUTO_INCREMENT = " . $number . ";");
			$pdo->queryDirect("ALTER TABLE " . $column['table_name'] . " AUTO_INCREMENT = " . $number);
			$count++;
		}
	}
}
echo $pdo->cli->header($count . " id columns reset");
