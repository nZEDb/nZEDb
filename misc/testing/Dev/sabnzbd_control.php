...
// --- Pause ---
if ($argv[2] === "pause") {
	echo $pdo->cli->header("Pausing sabnzbd.");
	$response = file_get_contents($saburl."api?mode=pause"."&apikey=".$sabapikey);
	echo $pdo->cli->header($response);
}

// --- Resume ---
if ($argv[2] === "resume") {
	echo $pdo->cli->header("Resuming sabnzbd.");
...
if ($argv[2] === "speedlimit" && isset($argv[3]) && is_numeric($argv[3])) {
	echo $pdo->cli->header("Speed limiting sabnzbd to ".$argv[3]." KB/s");
