	...
	while (($line = fgets($handle)) !== false) {
		if (preg_match('/(CREATE UNIQUE INDEX) (.+) ON (.+) ?\((.+)\);/i', $line, $match)) {
			...
			foreach ($columns as $column) {
				$check = $db->queryDirect("SHOW INDEXES IN " . trim($match[3]) . " WHERE non_unique = 0 AND column_name = '" . trim($column) . "'");
					...
					if (trim($match[3]) === 'collections') {
						...
						foreach ($tables as $row) {
							...
							if (preg_match('/collections_\d+/', $tbl)) {
								$check_collections = $db->queryDirect("SHOW INDEXES IN " . trim($tbl) . " WHERE non_unique = 0 AND column_name = '" . trim($column) . "'");
						...
						foreach ($tables as $row) {
							...
							if (preg_match('/binaries_\d+/', $tbl)) {
								$checkBinaries = $db->queryDirect("SHOW INDEXES IN " . trim($tbl) . " WHERE non_unique = 0 AND column_name = '" . trim($column) . "'");
						...
						foreach ($tables as $row) {
							...
							if (preg_match('/parts_\d+/', $tbl)) {
								$checkParts = $db->queryDirect("SHOW INDEXES IN " . trim($tbl) . " WHERE non_unique = 0 AND column_name = '" . trim($column) . "'");
						...
						foreach ($tables as $row) {
							...
							if (preg_match('/partrepair_\d+/', $tbl)) {
								$checkPartRepair = $db->queryDirect("SHOW INDEXES IN " . trim($tbl) . " WHERE non_unique = 0 AND column_name = '" . trim($column) . "'");
