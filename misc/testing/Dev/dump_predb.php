...
if (isset($argv[1]) && $argv[1] == 'export' && isset($argv[2])) {
	...
	echo $c->header("SELECT title, nfo, size, category, predate, source, md5, requestid, g.name FROM " . $table . " p LEFT OUTER JOIN groups g ON p.groupid = g.id INTO OUTFILE '" . $path . "' FIELDS TERMINATED BY '\\t\\t' ENCLOSED BY \"'\" LINES TERMINATED BY '\\r\\n';n");
	$db->queryDirect("SELECT title, nfo, size, category, predate, source, md5, requestid, g.name FROM " . $table . " p LEFT OUTER JOIN groups g ON p.groupid = g.id INTO OUTFILE '" . $path . "' FIELDS TERMINATED BY '\t\t' ENCLOSED BY \"'\" LINES TERMINATED BY '\r\n'");
	...
	echo $c->primary('INSERT INTO ' . $table . " (title, nfo, size, category, predate, source, md5, requestid, groupid) SELECT t.title, t.nfo, t.size, t.category, t.predate, t.source, t.md5, t.requestid, IF(g.id IS NOT NULL, g.id, 0) FROM tmp_pre t LEFT OUTER JOIN groups g ON t.groupname = g.name ON DUPLICATE KEY UPDATE predb.nfo = IF(predb.nfo is null, t.nfo, predb.nfo), predb.size = IF(predb.size is null, t.size, predb.size), predb.category = IF(predb.category is null, t.category, predb.category), predb.requestid = IF(predb.requestid = 0, t.requestid, predb.requestid), predb.groupid = IF(g.id IS NOT NULL, g.id, 0);\n");
	$db->queryDirect('INSERT INTO ' . $table . ' (title, nfo, size, category, predate, source, md5, requestid, groupid) SELECT t.title, t.nfo, t.size, t.category, t.predate, t.source, t.md5, t.requestid, IF(g.id IS NOT NULL, g.id, 0) FROM tmp_pre t LEFT OUTER JOIN groups g ON t.groupname = g.name ON DUPLICATE KEY UPDATE predb.nfo = IF(predb.nfo is null, t.nfo, predb.nfo), predb.size = IF(predb.size is null, t.size, predb.size), predb.category = IF(predb.category is null, t.category, predb.category), predb.requestid = IF(predb.requestid = 0, t.requestid, predb.requestid), predb.groupid = IF(g.id IS NOT NULL, g.id, 0)');
