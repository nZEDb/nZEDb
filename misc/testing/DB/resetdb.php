...
echo $pdo->cli->warning("This script removes all releases, nzb files, samples, previews , nfos, truncates all article tables and resets all groups.");
echo $pdo->cli->header("Are you sure you want reset the DB?  Type 'DESTROY' to continue:  \n");
echo $pdo->cli->warningOver("\n");
$line = fgets(STDIN);
if (trim($line) != 'DESTROY')
	exit($pdo->cli->error("This script is dangerous you must type DESTROY for it function."));

echo "\n";
echo $pdo->cli->header("Thank you, continuing...\n\n");
...
echo $pdo->cli->header("Deleting nzbfiles subfolders.");
try {
	...
	echo $pdo->cli->error($e->getMessage());
}

echo $pdo->cli->header("Deleting all images, previews and samples that still remain.");
try {
...
echo $pdo->cli->header("Getting Updated List of TV Shows from TVRage.");
...
if ($tvshows !== false) {
	...
	echo $pdo->cli->error("TVRage site has a hard limit of 400 concurrent api requests. At the moment, they have reached that limit. Please wait before retrying again.");
}

echo $pdo->cli->header("Deleted all releases, images, previews and samples. This script ran for " . $consoletools->convertTime(TIME() - $timestart));
