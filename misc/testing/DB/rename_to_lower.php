...
echo $pdo->cli->warning("This script renames all table colums to lowercase.");
echo $pdo->cli->header("Have you backed up your database? Type 'BACKEDUP' to continue:  \n");
echo $pdo->cli->warningOver("\n");
$line = fgets(STDIN);
if (trim($line) != 'BACKEDUP') {
	exit($pdo->cli->error("This script is dangerous you must type BACKEDUP for it function."));
}
...
if (count($list) == 0) {
	echo $pdo->cli->info("No table columns to rename");
} else {
	foreach ($list as $column) {
		if ($column['column_name'] !== strtolower($column['column_name'])) {
			echo $pdo->cli->header("Renaming Table " . $column['table_name'] . " Column " . $column['column_name']);
...
if ($count == 0) {
	echo $pdo->cli->info("All table column names are already lowercase");
} else {
	echo $pdo->cli->header($count . " colums renamed");
}
