...
if (isset($argv[1]) && $argv[1] === "all") {
	if (isset($argv[2]) && $argv[2] === "true") {
		...
		echo $pdo->cli->header("Resetting all postprocessing");
...
if (isset($argv[1]) && ($argv[1] === "consoles" || $argv[1] === "all")) {
	...
	if (isset($argv[2]) && $argv[2] === "true") {
		echo $pdo->cli->header("Resetting all Console postprocessing");
		$where = 'WHERE consoleinfoid IS NOT NULL';
	} else {
		echo $pdo->cli->header("Resetting all failed Console postprocessing");
...
if (isset($argv[1]) && ($argv[1] === "games" || $argv[1] === "all")) {
	...
	if (isset($argv[2]) && $argv[2] === "true") {
		echo $pdo->cli->header("Resetting all Games postprocessing");
		$where = ' WHERE gamesinfo_id != 0';
	} else {
		echo $pdo->cli->header("Resetting all failed Games postprocessing");
...
if (isset($argv[1]) && ($argv[1] === "movies" || $argv[1] === "all")) {
	...
	if (isset($argv[2]) && $argv[2] === "true") {
		echo $pdo->cli->header("Resetting all Movie postprocessing");
		$where = ' WHERE imdbid IS NOT NULL';
	} else {
		echo $pdo->cli->header("Resetting all failed Movie postprocessing");
...
if (isset($argv[1]) && ($argv[1] === "music" || $argv[1] === "all")) {
	...
	if (isset($argv[2]) && $argv[2] === "true") {
		echo $pdo->cli->header("Resetting all Music postprocessing");
		$where = ' WHERE musicinfoid IS NOT NULL';
	} else {
		echo $pdo->cli->header("Resetting all failed Music postprocessing");
...
if (isset($argv[1]) && ($argv[1] === "misc" || $argv[1] === "all")) {
	$ran = true;
	if (isset($argv[2]) && $argv[2] === "true") {
		echo $pdo->cli->header("Resetting all Additional postprocessing");
		...
		echo $pdo->cli->header("Resetting all failed Additional postprocessing");
	...
	echo $pdo->cli->primary("SELECT id FROM releases" . $where);
	$qry      = $pdo->queryDirect("SELECT id FROM releases" . $where);
	if ($qry !== false ) {
		$total = $qry->rowCount();
	} else {
...
if (isset($argv[1]) && ($argv[1] === "tv" || $argv[1] === "all")) {
	...
	if (isset($argv[2]) && $argv[2] === "true") {
		echo $pdo->cli->header("Resetting all TV postprocessing");
		$where = '  WHERE rageid != -1';
	} else {
		echo $pdo->cli->header("Resetting all failed TV postprocessing");
...
if (isset($argv[1]) && ($argv[1] === "books" || $argv[1] === "all")) {
	...
	if (isset($argv[2]) && $argv[2] === "true") {
		echo $pdo->cli->header("Resetting all Book postprocessing");
		$where = ' WHERE bookinfoid IS NOT NULL';
	} else {
		echo $pdo->cli->header("Resetting all failed Book postprocessing");
...
if (isset($argv[1]) && ($argv[1] === "xxx" || $argv[1] === "all")) {
	...
	if (isset($argv[2]) && $argv[2] === "true") {
		echo $pdo->cli->header("Resetting all XXX postprocessing");
		$where = ' WHERE xxxinfo_id != 0';
	} else {
		echo $pdo->cli->header("Resetting all failed XXX postprocessing");
...
if (isset($argv[1]) && ($argv[1] === "nfos" || $argv[1] === "all")) {
	...
	if (isset($argv[2]) && $argv[2] === "true") {
		echo $pdo->cli->header("Resetting all NFO postprocessing");
		$where = ' WHERE nfostatus != -1';
	} else {
		echo $pdo->cli->header("Resetting all failed NFO postprocessing");
		$where = " WHERE nfostatus < -1";
	}
...
if ($ran === false) {
	exit(
		$pdo->cli->error(
			...
			. "php $argv[0] xxx true         ...: To reset all xxx.\n"
