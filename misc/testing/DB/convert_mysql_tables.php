...
} elseif (isset($argv[1]) && $argv[1] == 'dmyisam') {
...
} elseif (isset($argv[1]) && $argv[1] == 'dinnodb') {
...
} elseif (isset($argv[1]) && $argv[1] == 'cinnodb') {
...
} elseif (isset($argv[1]) && $argv[1] == 'cinnodb-noparts') {
	$sql = 'SHOW TABLE STATUS WHERE (Engine != "InnoDB" OR Row_format != "Compressed") AND Engine != "SPHINX"';
	$tables = $pdo->query($sql);
	foreach ($tables as $row) {
		$tbl = $row['name'];
...
} elseif (isset($argv[1]) && $argv[1] == 'collections') {
