...
if (isset($argv[1]) && isset($argv[2]) && $argv[2] == "fmyisam") {
	...
	$tbl = $argv[1];
	if ($ftinnodb || (!$ftinnodb && $tbl !== 'release_search_data' && $tbl !== 'predb_hashes')) {
		printf($cli->header("Converting $tbl"));
		$pdo->queryExec("ALTER TABLE $tbl ENGINE=INNODB ROW_FORMAT=COMPRESSED");
	} else {
		printf($cli->header("Not converting release_search_data / predb_hashes as your INNODB version does not support fulltext indexes"));
	}
	...
	foreach ($tables as $row) {
		$tbl = $row['name'];
		if ($tbl !== 'release_search_data' && $tbl !== 'predb_hashes') {
			printf($cli->header("Converting $tbl"));
			$pdo->queryExec("ALTER TABLE $tbl ENGINE=INNODB ROW_FORMAT=DYNAMIC");
		}
	}
	if ($ftinnodb) {
		$sql = 'SHOW TABLE STATUS WHERE Name IN ("release_search_data", "predb_hashes") AND (Engine != "InnoDB" || Row_format != "Dynamic")';
		$tables = $pdo->query($sql);
		foreach ($tables as $row) {
			$tbl = $row['name'];
			printf($cli->header("Converting $tbl"));
		...
		printf($cli->header("Not converting release_search_data as your INNODB version does not support fulltext indexes"));
	}
	...
	foreach ($tables as $row) {
		$tbl = $row['name'];
		if ($tbl !== 'release_nfos' && $tbl !== 'release_search_data') {
			printf($cli->header("Converting $tbl"));
			$pdo->queryExec("ALTER TABLE $tbl ENGINE=INNODB ROW_FORMAT=COMPRESSED");
		}
	}
	...
	if ($ftinnodb) {
		$sql = 'SHOW TABLE STATUS WHERE Name IN ("release_search_data", "predb_hashes") AND (Engine != "InnoDB" || Row_format != "Compressed")';
		$tables = $pdo->query($sql);
		foreach ($tables as $row) {
			$tbl = $row['name'];
			printf($cli->header("Converting $tbl"));
	...
	foreach ($tables as $row) {
		$tbl = $row['name'];
		if ($tbl !== 'release_nfos' && $tbl !== 'release_search_data' && !preg_match('/parts/', $tbl)) {
			printf($cli->header("Converting $tbl"));
			$pdo->queryExec("ALTER TABLE $tbl ENGINE=INNODB ROW_FORMAT=COMPRESSED");
		}
	}
	...
	exit($cli->error(
		...
		. "                                                                                 NB if your innodb version < 5.6 release_search_data / predb_hashes will not be converted as fulltext indexes are not supported.\n"
