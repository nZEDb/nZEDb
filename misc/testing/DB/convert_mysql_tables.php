...
if (isset($argv[1]) && isset($argv[2]) && $argv[2] == "fmyisam") {
	...
	foreach ($tables as $row) {
		$tbl = $row['name'];
		if ($tbl !== 'release_search_data' && $tbl !== 'predb_hashes' && $tbl !== 'bookinfo' && $tbl !== 'consoleinfo' && $tbl !== 'musicinfo') {
			printf($cli->header("Converting $tbl"));
			$pdo->queryExec("ALTER TABLE $tbl ENGINE=INNODB ROW_FORMAT=DYNAMIC");
		}
	}
	if ($ftinnodb) {
		$sql = 'SHOW TABLE STATUS WHERE Name IN ("release_search_data", "predb_hashes", "bookinfo", "consoleinfo", "musicinfo") AND (Engine != "InnoDB" || Row_format != "Dynamic")';
		$tables = $pdo->query($sql);
		foreach ($tables as $row) {
			$tbl = $row['name'];
			printf($cli->header("Converting $tbl"));
	...
	if ($ftinnodb) {
		$sql = 'SHOW TABLE STATUS WHERE Name IN ("release_search_data", "predb_hashes", "bookinfo", "consoleinfo", "musicinfo") AND (Engine != "InnoDB" || Row_format != "Compressed")';
		$tables = $pdo->query($sql);
		foreach ($tables as $row) {
			$tbl = $row['name'];
			printf($cli->header("Converting $tbl"));
