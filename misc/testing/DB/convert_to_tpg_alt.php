...
if ($collections_rows instanceof \Traversable) {
	foreach ($collections_rows as $row) {
		...
		//collection
		$pdo->queryExec("INSERT IGNORE INTO collections_" . $row['group_id'] . " (subject, fromname, date, xref, totalfiles, group_id, collectionhash, dateadded, filecheck, filesize, releases_id) "
			. "SELECT subject, fromname, date, xref, totalfiles, group_id, collectionhash, dateadded, filecheck, filesize, releases_id FROM collections WHERE group_id = ${row['group_id']}");
