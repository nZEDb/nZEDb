...
function getOddGames()
{
	...
	if ($res !== false) {
		$pdo->log::out('Processing... 150 release(s).', 'header', true);
		...
		foreach ($res as $arr) {
			...
			if ($gameInfo !== false) {
					$pdo->log::out(
						$pdo->log::header('Looking up: ', false) .
						$pdo->log::primary($gameInfo['title'], true)
					);
			...
			$pdo->log::out('No games in 0day/ISO to process.', 'header', true);
	}
}
