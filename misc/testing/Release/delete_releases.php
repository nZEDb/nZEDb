...
if ($totalArgs < 2) {
	exit($cli->info($n .
		...
		'categoriesid : Look for releases within specified category (modifiers: equals)' . $n .
		'imdbid     : Look for releases with imdbid (modifiers: equals)' . $n .
		'videosid  : Look for releases with videos_id (modifiers: equals)' . $n .
		...
		$_SERVER['_'] . ' ' . $argv[0] . ' imdbid=equals=NULL categoriesid=equals=2999 nzbstatus=equals=1 adddate=bigger=2880 # Remove other movie releases with non-cleaned names added > 120 days ago'
	));
}
