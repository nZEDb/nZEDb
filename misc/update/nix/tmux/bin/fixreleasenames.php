...
if (!isset($argv[1])) {
	...
	if (isset($pieces[1]) && $pieces[0] == 'nfo') {
	...
	} elseif (isset($pieces[1]) && $pieces[0] == 'filename') {
		$release = $pieces[1];
	...
	} elseif (isset($pieces[1]) && $pieces[0] == 'md5') {
		$release = $pieces[1];
		if ($res = $pdo->queryOneRow(sprintf('SELECT r.id AS releases_id, r.name, r.searchname, r.categories_id, r.groups_id, dehashstatus, rf.name AS filename '
			...
			} elseif (preg_match('/[a-fA-F0-9]{32,40}/i', $res['filename'], $matches)) {
				$namefixer->matchPredbHash($matches[0], $res, 1, 1, true, 1);
			} else {
	...
	} elseif (isset($pieces[1]) && $pieces[0] == 'par2') {
	...
	} elseif (isset($pieces[1]) && $pieces[0] == 'miscsorter') {
	...
	} elseif (isset($pieces[1]) && $pieces[0] == 'predbft') {
		$pre = $pieces[1];
