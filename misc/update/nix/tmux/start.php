...
if (count($session) !== 0) {
	exit($pdo->cli->error("tmux session:" . $tmux_session . " is already running, aborting.\n"));
} else {
...
foreach ($apps as &$value) {
	if (!command_exist($value)) {
		exit($pdo->cli->error("Tmux scripts require " . $value . " but it's not installed. Aborting.\n"));
	}
}
...
echo $pdo->cli->header("Resetting expired collections dateadded to now. This could take a minute or two. Really.");
if ($tablepergroup == 1) {
