...
switch ($options[1]) {
	...
	case 'get_range':
		...
		$groupMySQL = (new Groups)->table->getAllByName($options[3]);
		...
		$return = $binaries->scan(
			...
			$options[4],
			$options[5],
			(Settings::value('..safepartrepair') == 1 ? 'update' : 'backfill')
		);
		if (empty($return)) {
			exit();
		}
	...
	case 'part_repair':
		$groups = new Groups(['Settings' => $pdo]);
		$groupMySQL = (new Groups)->table->getAllByName($options[2]);
		$nntp = nntp($pdo);
...
function &nntp(&$pdo, $alternate = false)
{
	...
	if (($alternate && Settings::value('..alternate_nntp') == 1 ? $nntp->doConnect(true, true) :
			$nntp->doConnect()) !== true) {
