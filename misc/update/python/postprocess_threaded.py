...
if sys.argv[1] == "additional":
	cur[0].execute("SELECT LEFT(r.guid, 1) AS group_id FROM releases r LEFT JOIN category c ON c.id = r.categoryid WHERE r.nzbstatus = 1 "+maxsize+" AND (r.haspreview = -1 AND c.disablepreview = 0) AND r.passwordstatus BETWEEN -6 AND -1 GROUP BY LEFT(r.guid, 1)")
	datas = cur[0].fetchall()
elif sys.argv[1] == "nfo":
	cur[0].execute("SELECT LEFT(guid, 1) AS group_id FROM releases WHERE nzbstatus = 1 AND nfostatus BETWEEN -6 AND -1 GROUP BY LEFT(guid, 1)")
	datas = cur[0].fetchall()
elif sys.argv[1] == "movie" and len(sys.argv) == 3 and sys.argv[2] == "clean":
	cur[0].execute("SELECT LEFT(guid, 1) AS group_id FROM releases WHERE nzbstatus = 1 AND isrenamed = 1 AND searchname IS NOT NULL AND imdbid IS NULL AND categoryid BETWEEN 2000 AND 2999 GROUP BY LEFT(guid, 1) "+orderBY)
	datas = cur[0].fetchall()
elif sys.argv[1] == "movie":
	cur[0].execute("SELECT LEFT(guid, 1) AS group_id FROM releases WHERE nzbstatus = 1 AND searchname IS NOT NULL AND imdbid IS NULL AND categoryid BETWEEN 2000 AND 2999 GROUP BY LEFT(guid, 1) "+orderBY)
	datas = cur[0].fetchall()
elif sys.argv[1] == "tv" and len(sys.argv) == 3 and sys.argv[2] == "clean":
	cur[0].execute("SELECT LEFT(guid, 1) AS group_id FROM releases WHERE nzbstatus = 1 AND isrenamed = 1 AND searchname IS NOT NULL AND rageid = -1 AND categoryid BETWEEN 5000 AND 5999 GROUP BY LEFT(guid, 1) "+orderBY)
	datas = cur[0].fetchall()
elif sys.argv[1] == "tv":
	cur[0].execute("SELECT LEFT(guid, 1) AS group_id FROM releases WHERE nzbstatus = 1 AND searchname IS NOT NULL AND rageid = -1 AND categoryid BETWEEN 5000 AND 5999 GROUP BY LEFT(guid, 1) "+orderBY)
	datas = cur[0].fetchall()
