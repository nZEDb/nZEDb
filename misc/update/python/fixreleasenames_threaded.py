...
if len(sys.argv) > 1 and sys.argv[1] == "nfo":
	...
	while len(datas) == 0 and maxtries >= -5:
		run = "SELECT DISTINCT rel.id FROM releases rel INNER JOIN releasefiles rf ON rel.id = rf.releaseid WHERE nzbstatus = 1 AND isrenamed = 0 AND rel.dehashstatus BETWEEN %s AND 0 AND rel.passwordstatus >= -1 AND (rel.ishashed = 1 OR rf.name REGEXP '[a-fA-F0-9]{32,40}') ORDER BY postdate ASC LIMIT %s"
