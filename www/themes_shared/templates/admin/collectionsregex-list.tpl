	...
	<table style="margin-top:10px;" class="data Sortable highlight">
		...
		{foreach from=$regex item=row}
			<tr id="row-{$row.id}" class="{cycle values=",alt"}">
				...
				<td title="Edit this regex"><a href="{$smarty.const.WWW_TOP}/collectionsregex-edit.php?id={$row.id}">Edit</a></td>
				...
				{/if}
				<td title="Edit this regex"><a href="{$smarty.const.WWW_TOP}/collectionsregex-edit.php?id={$row.id}">{$row.regex|escape:html|truncate:50:"...":true}</a></td>
			</tr>
		{/foreach}
	</table>
