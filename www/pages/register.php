...
if ($page->pdo->getSetting('registerstatus') == Sites::REGISTER_STATUS_CLOSED || $page->pdo->getSetting('registerstatus') == Sites::REGISTER_STATUS_API_ONLY) {
	$page->smarty->assign('error', "Registrations are currently disabled.");
	$showregister = 0;
} elseif ($page->pdo->getSetting('registerstatus') == Sites::REGISTER_STATUS_INVITE && (!isset($_REQUEST["invitecode"]) || empty($_REQUEST['invitecode']))) {
