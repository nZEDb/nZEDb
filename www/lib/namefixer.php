...
class Namefixer
{
	...
	public function fileCheck($release, $echo, $type, $namestatus)
	{
		...
		if (preg_match('/^([a-z0-9\.\-_]+(19|20)\d\d[a-z0-9\.\-_]+[\.\-_ ](720p|1080p|BDRip|bluray|DVDRip|x264|XviD)[a-z0-9\.\-_]+)\.[a-z]{2,}$/i', $release["textstring"], $result) && $this->relid !== $release["releaseID"])
		...
		if (preg_match('/.+\\\\(.+\((19|20)\d\d\)\.iso)/i', $release["textstring"], $result) && $this->relid !== $release["releaseID"])
		{
