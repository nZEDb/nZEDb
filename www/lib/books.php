 ...
 */
 ...
 class Books
 {
	...
	public function processBookReleases($threads=1)
	{
		...
		$res = $db->queryDirect(sprintf("SELECT searchname, ID from releases where bookinfoID IS NULL and nzbstatus = 1 and relnamestatus not in (0, 1) and  categoryID = 8010 order by postdate desc LIMIT %d,%d", floor(($this->bookqty) * ($threads * 1.5)), $this->bookqty));
