	...
	*/
	...
	public function collectionsCleaner($subject, $groupID="", $nofiles=false)
	{
		...
		else if ($groupName === "alt.binaries.anime")
		{
			...
			else if (preg_match('/^<TOWN> www\.town\.ag > sponsored by www\.ssl-news\.info > \(\d+(\/\d+\) ".+?)'.$this->ext.' - \d+[,.]\d+ [mMkKgG][bB] - yEnc$/', $subject, $match))
				return $match[1];
		...
		else if ($groupName === "alt.binaries.ath")
		{
			...
			else if (preg_match('/^(>+Hell-of-Usenet(\.org)?>+( -)? \[)\d+\/\d+\] - "(.+?)'.$this->ext.'( - \d+[.,]\d+ [kKmMgG][bB])? yEnc$/', $subject, $match))
		...
		else if ($groupName === "alt.binaries.audio.warez")
		{
			...
			else if (preg_match('/^\((\?{4}\) \[)\d+(\/\d+\] - ".+?)'.$this->ext.' yEnc$/', $subject, $match))
				return $match[1].$match[2];
		...
		else if ($groupName === "alt.binaries.b4e")
		{
			...
			else if (preg_match('/^(- ".+?)'.$this->ext.' yEnc$/', $subject, $match))
				return $match[1];
			else
				return $this->collectionsCleanerHelper($subject, $groupName, $nofiles);
		}
		else if ($groupName === "alt.binaries.barbarella")
		{
			...
			else if (preg_match('/^\(\d+(\/\d+\) - .+? - ".+?)( \(\d+\))?'.$this->ext.' - \d+[,.]\d+ [mMkKgG][bB] - yEnc$/', $subject, $match))
				return $match[1];
			...
			else if (preg_match('/^(>>> www\.lords-of-usenet\.org <<<.+? ".+?)'.$this->ext.' .+? \[\d+\/\d+\] - .+? yEnc$/', $subject, $match))
				return $match[1];
			...
			else if (preg_match('/^.+? (-|\(PW\))\s+\[.+? -\] \[\d+[,.]\d+ [mMkKgG][bB]\] \[\d+(\/\d+\] ".+?)'.$this->ext.' yEnc$/', $subject, $match))
				return $match[2];
			...
			else if (preg_match('/^.+?\s{2,}\d+[,.]\d+ [mMkKgG]([bB]\s{2,}".+?)'.$this->ext.'\s{2,}(\d+ B|\d+[,.]\d+ [mMkKgG][bB]) yEnc$/', $subject, $match))
				return$match[1];
			//(MKV - DVD - Rip - German - English - Italiano) - "CALIGULA (1982) UNCUT.sfv" yEnc
			else if (preg_match('/^(\(.+?\) - ".+?)'.$this->ext.' yEnc$/', $subject, $match))
				return $match[1];
			//"sre56565ztrtzuzi8inzufft.par2" yEnc
			else if (preg_match('/^"([a-z0-9]+)'.$this->ext.' yEnc$/', $subject, $match))
				return $match[1];
			else
				return $this->collectionsCleanerHelper($subject, $groupName, $nofiles);
		}
		else if ($groupName === "alt.binaries.big")
		{
			...
			else if (preg_match('/^\(\d+(\/\d+\) - ".+?)'.$this->ext.' yEnc$/', $subject, $match))
				return $match[1];
			...
			else if (preg_match('/^\(\d+(\/\d+\) ".+?)'.$this->ext.' - \d+[,.]\d+ [mMkKgG][bB] -(re)? yEnc$/', $subject, $match))
				return $match[1];
			...
			else if (preg_match('/^\([a-z]+\) \[\d+(\/\d+\] - ".+?)'.$this->ext.' yEnc$/', $subject, $match))
				return $match[1];
		...
		else if ($groupName === "alt.binaries.bloaf")
		{
			...
			else if (preg_match('/^\[\d+(\/\d+\] - ".+?)'.$this->ext.' - \d+[,.]\d+ [mMkKgG][bB] .+? usenet-space.+?yEnc$/', $subject, $match))
				return $match[1];
			...
			else if (preg_match('/^\(\d+(\/\d+\) ".+?)'.$this->ext.' - \d+[,.]\d+ [mMkKgG][bB] - yEnc$/', $subject, $match))
				return $match[1];
		...
		else if ($groupName === "alt.binaries.blu-ray")
		{
			...
			else if (preg_match('/^.+?<<\d+\/\d+>> "(.+?)'.$this->ext.' - \d+[.,]\d+ [kKmMgG][bB] - yEnc$/', $subject, $match))
				return $match[1];
			else
				return $this->collectionsCleanerHelper($subject, $groupName, $nofiles);
		}
		else if ($groupName === "alt.binaries.boneless")
		{
			...
			else if (preg_match('/^([A-Za-z0-9].+? \((19|20)\d\d\) ")\d{2}\. .+?'.$this->ext.' yEnc$/', $subject, $match))
				return $match[1];
			//"BB636.part14.rar" - (15/39) - yEnc
			else if (preg_match('/^"([a-zA-Z0-9]+)'.$this->ext.' - \(\d+\/\d+\) - yEnc$/', $subject, $match))
				return $match[1];
			...
			else if (preg_match('/^(.+?\.(info|org)>+ - \[)\d+\/\d+\] - "(.+?)'.$this->ext.' yEnc$/', $subject, $match))
				return $match[1].$match[3];
			...
			else if (preg_match('/^\(.+?\(PWP\).+?\) \(\d+(\/\d+\) ".+?)'.$this->ext.' .+? \d+[,.]\d+ [mMkKgG][bB] .+ yEnc$/', $subject, $match))
				return $match[1];
		...
		else if ($groupName === "alt.binaries.documentaries")
		{
			...
			if (preg_match('/^(#sterntuary - .+? - ".+?)'.$this->ext.' yEnc$/', $subject, $match))
				return $match[1];
			else
				return $this->collectionsCleanerHelper($subject, $groupName, $nofiles);
		}
		...
		else if ($groupName === "alt.binaries.erotica")
		{
			...
			else if (preg_match('/^<TOWN><www\.town\.ag > <download all our files with>>> www\.ssl-news\.info <<< > \[\d+(\/\d+\] - ".+?)'.$this->ext.' - /', $subject, $match))
				return $match[1];
		...
		else if ($groupName === "alt.binaries.etc")
		{
			...
			if (preg_match('/^(\d+ - \[)\d+\/\d+\] - ".+?'.$this->ext.' yEnc$/', $subject, $match))
				return $match[1];
			else
				return $this->collectionsCleanerHelper($subject, $groupName, $nofiles);
		}
		...
		else if ($groupName === "alt.binaries.hdtv.x264")
		{
			...
			if (preg_match('/^\(\d+(\/\d+\) ".+?)'.$this->ext.' - \d+[,.]\d+ [mMkKgG][bB] - yEnc$/', $subject, $match))
				return $match[1];
		...
		else if ($groupName === "alt.binaries.highspeed")
		{
			...
			else if (preg_match('/^www\..+? \[Sponsored.+?\] \(\d+(\/\d+\) ".+?)'.$this->ext.' yEnc$/', $subject, $match))
				return $match[1];
		...
		else if ($groupName === "alt.binaries.inner-sanctum")
		{
			...
			if (preg_match('/^([a-fA-F0-9]+) \[\d+\/\d+\] - ".+?'.$this->ext.' yEnc$/', $subject, $match))
				return $match[1];
		...
		else if ($groupName === "alt.binaries.mom")
		{
			...
			else if (preg_match('/(.+?\.net <<<Partner von SSL-News\.info>>> - \[)\d+(\/\d+\] - ".+?)'.$this->ext.' yEnc$/', $subject, $match))
				return $match[1].$match[2];
			//<ghost-of-usenet.org>XCOM.Enemy.Unknown.Deutsch.Patch.TokZic [0/9] - "XCOM Deutsch.nzb" ein CrazyUpp yEnc
			else if (preg_match('/^(<ghost-of-usenet\.org>.+? \[)\d+\/\d+\] - ".+?" .+? yEnc$/', $subject, $match))
				return $match[1];
		...
		else if ($groupName === "alt.binaries.moovee")
		{
			...
			else if (preg_match('/^(\[\d+\]-\[.+?\]-\[.+?\]- ").+?'.$this->ext.' yEnc$/', $subject, $match))
				return $match[1];
			...
			else if (preg_match('/^\[x+\]-\[.+?\]-\[ x+ \]-\[\d+(\/\d+\] - ".+?)'.$this->ext.' yEnc$/', $subject, $match))
				return $match[1];
			...
			else if (preg_match('/^\[\d+\]-\[\d+(\/\d+\] - ".+?)'.$this->ext.' yEnc$/', $subject, $match))
				return $match[1];
		...
		else if ($groupName === "alt.binaries.movies.divx")
		{
			...
			if (preg_match('/^(\(www\.Thunder-News\.org\) .+? - ".+?)'.$this->ext.' yEnc$/', $subject, $match))
				return $match[1];
			//Movieland Post Voor FTN - [01/43] - "movieland0560.par2" yEnc
			if (preg_match('/^([a-zA-Z ]+Post Voor FTN - \[\d+\/\d+\] - ".+?)'.$this->ext.' yEnc$/', $subject, $match))
				return $match[1];
			//Disney short films collection by mayhem masta"1923 - Alice's Wonderland.vol15+7.par2" yEnc
			else if (preg_match('/(.+?by mayhem masta".+?)'.$this->ext.' yEnc$/', $subject, $match))
				return $match[1];
			else
				return $this->collectionsCleanerHelper($subject, $groupName, $nofiles);
		}
		...
		else if ($groupName === "alt.binaries.multimedia.anime")
		{
			...
			if (preg_match('/(.+? \((360|480|720|1080)p\|.+? ~bY .+? \[)\d+\/\d+\] - ".+?\[[A-F0-9]+\].+?'.$this->ext.' yEnc$/', $subject, $match))
				return $match[1];
			else
				return $this->collectionsCleanerHelper($subject, $groupName, $nofiles);
		}
		...
		else if ($groupName === "alt.binaries.multimedia.documentaries")
		{
			...
			if (preg_match('/^(".+?)'.$this->ext.' - \[\d+ of \d+ - \d+ [kKmMgG][bB]\] yEnc$/', $subject, $match))
				return $match[1];
			else
				return $this->collectionsCleanerHelper($subject, $groupName, $nofiles);
		}
		else if ($groupName === "alt.binaries.multimedia.scifi")
		{
			...
			if (preg_match('/^(some m4vs - ".+?)'.$this->ext.' yEnc$/', $subject, $match))
				return $match[1];
			else
				return $this->collectionsCleanerHelper($subject, $groupName, $nofiles);
		}
		...
		else if ($groupName === "alt.binaries.sounds.lossless")
		{
			...
			if (preg_match('/^http:\/\/dream-of-usenet\.org .+? - \[\d+(\/\d+\] - ".+?)'.$this->ext.' yEnc$/', $subject, $match))
				return $match[1];
			...
			else if (preg_match('/^\([A-Z0-9][a-zA-Z0-9 ]+\) \[\d+(\/\d+\] - ".+?)'.$this->ext.' yEnc$/', $subject, $match))
				return $match[1];
			else
				return $this->collectionsCleanerHelper($subject, $groupName, $nofiles);
		}
		else if ($groupName === "alt.binaries.sounds.mp3")
		{
			...
			if (preg_match('/^\(dream-of-usenet\.info\) - \[\d+(\/\d+\] - ".+?)'.$this->ext.' yEnc$/', $subject, $match))
				return $match[1];
			//http://dream-of-usenet.org empfehlen newsconnection.eu - [02/32] - "Adam_Ant-Manners_and_Physique-(MCAD-6315)-CD-FLAC-1989-2Eleven.par2" yEnc
			else if (preg_match('/^http:\/\/dream-of-usenet\.org .+? - \[\d+(\/\d+\] - ".+?)'.$this->ext.' yEnc$/', $subject, $match))
				return $match[1];
			...
			else if (preg_match('/^(.+?usenet-space.+?Powered by.+? ".+?)'.$this->ext.'.+? \d+\/\d+ \(\d+[.,]\d+ [kKmMgG][bB]\) .+? \d+[.,]\d+ [kKmMgG][bB] .+?yEnc$/', $subject, $match))
				return $match[1];
			else
				return $this->collectionsCleanerHelper($subject, $groupName, $nofiles);
		}
		else if ($groupName === "alt.binaries.teevee")
		{
			...
			else if (preg_match('/^\(\d+(\/\d+\) ".+?)'.$this->ext.' - \d.+?B - (\d.+?B -)? yEnc$/', $subject, $match))
				return $match[1];
			//[01/42] - "King.And.Maxwell.S01E08.1080p.WEB-DL.DD5.1.H264-Abjex.par2" yEnc
			else if (preg_match('/^\[\d+(\/\d+\] - "[A-Za-z0-9.-]+?)'.$this->ext.' yEnc$/', $subject, $match))
				return $match[1];
			...
			else if (preg_match('/^\(bf1\) \[\d+(\/\d+\] - ".+?)'.$this->ext.' yEnc$/', $subject, $match))
				return $match[1];
			...
			else if (preg_match('/^(\[ [A-Za-z]+ \] - (\[\d+\/\d+\] - )?".+?)'.$this->ext.' yEnc$/', $subject, $match))
				return $match[1];
			...
			else if (preg_match('/^(\(www\..+?\) .+? <Sponsored.+?> - ".+?)'.$this->ext.' yEnc$/', $subject, $match))
				return $match[1];
			...
			else if (preg_match('/(\.info .+?Powered by .+?\.com ".+?)'.$this->ext.' .+? \d+\/\d+ \(\d+[,.]\d+ [mMkKgG][bB]\) .+? yEnc$/', $subject, $match))
				return $match[1];
		...
		else if ($groupName === "alt.binaries.town")
		{
			...
			if (preg_match('/town\.ag.+?download all our files with.+?www\..+?\.info.+? \[\d+(\/\d+\] - ".+?)(-sample)?'.$this->ext.' - \d+[.,]\d+ [kKmMgG][bB] yEnc$/', $subject, $match))
				return $match[1];
			else
				return $this->collectionsCleanerHelper($subject, $groupName, $nofiles);
		}
	...
	public function collectionsCleanerHelper($subject, $groupName, $nofiles)
	{
		...
		if (!preg_match('/\.(flac|lossless|mp3|music|sounds)/', $groupName))
		{
			...
			// File extensions.
			$cleansubject = preg_replace('/'.$this->ext.'/i', ' ', $cleansubject);
	...
	public function releaseCleaner($subject, $groupID)
	{
		...
		else if ($groupName === "alt.binaries.anime")
		{
			...
			//<TOWN> www.town.ag > sponsored by www.ssl-news.info > (1/3) "HolzWerken_40.par2" - 43,89 MB - yEnc
				...
				return $match[1];
			//(1/9)<<<www.town.ag>>> sponsored by ssl-news.info<<<[HorribleSubs]_AIURA_-_01_[480p].mkv "[HorribleSubs]_AIURA_-_01_[480p].par2" yEnc
			else if (preg_match('/^\(\d+\/\d+\).+?www\.town\.ag.+?sponsored by (www\.)?ssl-news\.info<+?.+? "(.+?)'.$this->ext.' yEnc$/', $subject, $match))
				return $match[2];
		...
		else if ($groupName === "alt.binaries.ath")
		{
			...
			else if (preg_match('/^<Have Fun> "(.+?)'.$this->ext.' SpongeBoZZ yEnc$/', $subject, $match))
				return $match[1].$match[2];
		...
		else if ($groupName === "alt.binaries.classic.tv.shows")
		{
			...
			else if (preg_match('/^[a-zA-Z0-9][a-zA-Z0-9.-]+S\d+E\d+([a-zA-Z0-9.]+)? \[\d+\/\d+\] - "(.+?)'.$this->ext.' yEnc$/', $subject, $match))
				return $match[2];
			//"Batman - S1E13-The Thirteenth Hat.par2" yEnc
			//"The Munsters - 1x01 Munster Masquerade.part.par" HQ DVDRip[02/16] yEnc
			else if (preg_match('/^(Re: )?"(.+?)(\.avi|\.mkv)?'.$this->ext.'( HQ DVDRip\[\d+\/\d+\])? yEnc$/', $subject, $match))
				return $match[2];
			//Re: Outside Edge series 1 - [01/20] - "Outside Edge S01.nfo" yEnc
			//Green Acres Season 1 [01/87] - "Green Acres Season 1.par2" yEnc
			//MASH Season 1 - [01/54] - "MASH - Season 01.par2" yEnc
			else if (preg_match('/^(Re: )?[a-zA-Z0-9]+.+? (series|Season) \d+ (- )?\[\d+\/\d+\] - "(.+?)'.$this->ext.' yEnc$/', $subject, $match))
				return $match[4];
			...
			else if (preg_match('/^(REQ - )?[A-Z0-9a-z][A-Z0-9a-z.]+ - \d+x\d+ (- [A-Z0-9a-z.]+ )?- \[\d+\/\d+\] - "(.+?)(\.avi|\.mkv)?'.$this->ext.' yEnc$/', $subject, $match))
				return $match[3];
			else
				return $this->releaseCleanerHelper($subject);
		}
		...
		else if ($groupName === "alt.binaries.cores")
		{
			...
			if (preg_match('/^Film - (\[\d+\/\d+\] - )?"(.+?)'.$this->ext.' yEnc$/', $subject, $match))
				return $match[2];
			...
			else if (preg_match('/^[a-zA-Z0-9]+ post voor u op www\..+? - \[\d+\/\d+\] - "(.+?)'.$this->ext.' yEnc$/', $subject, $match))
				return $match[1];
			...
			else if (preg_match('/^>+ .+?\.info [<>+]+ .+?\.com <+ "(.+?)\s+- .*?'.$this->ext.' - .+? yEnc$/', $subject, $match))
				return $match[1];
			...
			else if (preg_match('/^[A-Za-z]+ - \[\d+\/\d+\] - "\d+-(.+?)'.$this->ext.' yEnc$/', $subject, $match))
				return $match[1];
			else
				return $this->releaseCleanerHelper($subject);
		}
		...
		else if ($groupName === "alt.binaries.hdtv.x264")
		{
			...
			else if (preg_match('/^(AMEa)?\(\d+\/\d+\)( - AME-)? "(.+?)'.$this->ext.' - \d+[,.]\d+ [mMkKgG][bB] - yEnc$/', $subject, $match))
				return $match[3];
			//Hard.Target.1993.1080p.Bluray.X264-BARC0DE - [36/68] - "BARC0DE1080pHTAR.r22" yEnc
			//Goddess.2013.720p.BDRip.x264.AC3-noOne  [086/100] - "Goddess.2013.720p.BDRip.x264.AC3-noOne.part84.rar" yEnc
			else if (preg_match('/^([A-Z0-9a-z][A-Za-z0-9.-]+) -? \[\d+\/\d+\] - ".+?" yEnc$/', $subject, $match))
			...
			else if (preg_match('/^[A-Z][a-zA-Z0-9 ]+ [- ] "(.+?)'.$this->ext.' yEnc$/', $subject, $match))
				return $match[1];
			//"Ninja-Revenge Will Rise UC-Pittis AVCHD-ADD.English.dtsHR.nfo.txt" (01/55) yEnc
			else if (preg_match('/^"(.+?)'.$this->ext.' \(\d+\/\d+\) yEnc$/', $subject, $match))
				return $match[1];
		...
		else if ($groupName === "alt.binaries.highspeed")
		{
			...
			else if (preg_match('/^\[\d+\/\d+\] - "(.+?) upp.by.+?'.$this->ext.' yEnc$/', $subject, $match))
				return $match[1];
			...
			else if (preg_match('/^\[ .+? \] \[ www\..+? \]( \[.+?\])? \[\d+\/\d+\] - "(.+?)'.$this->ext.' yEnc$/', $subject, $match))
				return $match[2];
		...
		else if ($groupName === "alt.binaries.movies.divx")
		{
			...
			if (preg_match('/^\[\d+\]-\[.+?\]-\[.+?\]-\[ (.+?) \]-\[\d+\/\d+\] - ".+?" yEnc$/', $subject, $match))
				return $match[1];
			//(www.Thunder-News.org) >CD2< <Sponsored by Secretusenet> - "exvid-emma-cd2.par2" yEnc
			else if (preg_match('/^\(www\.Thunder-News\.org\) .+? - "(.+?)'.$this->ext.' yEnc$/', $subject, $match))
				return $match[1];
			//Movieland Post Voor FTN - [01/43] - "movieland0560.par2" yEnc
			else if (preg_match('/^[a-zA-Z ]+Post Voor FTN - \[\d+\/\d+\] - "(.+?)'.$this->ext.' yEnc$/', $subject, $match))
				return $match[1];
			//Disney short films collection by mayhem masta"1923 - Alice's Wonderland.vol15+7.par2" yEnc
			else if (preg_match('/.+?by mayhem masta"(.+?)'.$this->ext.' yEnc$/', $subject, $match))
				return $match[1];
		...
		else if ($groupName === "alt.binaries.sounds.lossless")
		{
			...
			else if (preg_match('/^trtk\d+ - \[\d+\/\d+\] - "(.+?)'.$this->ext.' yEnc$/', $subject, $match))
				return $match[1];
