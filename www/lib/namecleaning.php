...
class nameCleaning
{
    ...
    */
    public function releaseCleaner($subject, $groupName)
    {
        ...
        {
            //[f3a543495657d38c361dbe767a8506df] - sandramilka01-casting [10/25] - "sandramilka01-casting.part08.rar" yEnc
            if (preg_match('/\[([a-fA-F0-9]+)][-_ ]{0,3}.+?[-_ ]{0,3}[\(\[]\d+\/(\d+[\)\]])[-_ ]{0,3}"(.+)(\.part\d*|\.rar)?(\.vol.+ \(\d+\/\d+\) "|\.[A-Za-z0-9]{2,4}") yEnc$/', $subject, $match))
                return $match[1];
            //[278997]-[FULL]-[#a.b.erotica]-[ chi-the.walking.dead.xxx ]-[06/51] - "chi-the.walking.dead.xxx-s.mp4" yEnc
            //[######]-[FULL]-[#a.b.teevee@EFNet]-[ Misfits.S01.SUBPACK.DVDRip.XviD-P0W4DVD ] [1/5] - "Misfits.S01.SUBPACK.DVDRip.XviD-P0W4DVD.nfo" yEnc
            else if (preg_match('/\[[\d#]+\]-\[.+?\]-\[.+?\]-\[ (.+?) \][- ]\[\d+\/\d+\] - ".+?" yEnc$/', $subject, $match))
        ...
        else if ($groupName === "alt.binaries.inner-sanctum")
        {
            ...
            //[28148]-[FULL]-[#a.b.inner-sanctum@EFNET]-[ Chk_Chk_Chk--Jamie_My_Intentions_Are_Bass-(WAP310D)-WEB-2010-OMA ]-[01/19] - "00-chk_chk_chk--jamie_my_intentions_are_bass-(wap310d)-web-2010-oma.jpg" yEnc
                ...
                return $match[1];
            //[ nEwZ[NZB].iNFO - [ Zed--The_Invitation-WEB-2010-WUS ] - File [12/13]: "08-zed--the_river.mp3" yEnc
            else if (preg_match('/^\[ nEwZ\[NZB\]\.iNFO( \])?[-_ ]{0,3}\[ (.+?) \][-_ ]{0,3}(File )?[\(\[]\d+\/(\d+[\)\]]): "(.+)(\.part\d*|\.rar)?(\.vol.+ \(\d+\/\d+\) "|\.[A-Za-z0-9]{2,4}").+?(yEnc)?$/', $subject, $match))
                return $match[2];
            //nEwZ[NZB].iNFO - VA-Universal_Music_Sampler_07_February-PROMO-CDR-FLAC-2013-WRE - File [6/9]: "01-alesso-years_(hard_rock_sofa_remix).flac"
        ...
        else if ($groupName === "alt.binaries.teevee")
        {
            ...
            else if (preg_match('/^<<<(Hustensaft\d+|Nimue|CowboyUp\d+(-\d+)?( Serie)?)>>><<<(.+)>>>.+<<<Powered by.+yEnc$/i', $subject, $match))
                return $match[3];
            ...
            else if (preg_match('/^(tv )?(.+(\d+x\d+|Season|S\d+|1080|720).+)\..+(\[\d+\/\d+\]|File \d+ of \d+:)[ -]*("|#34;).+("|#34;) yEnc$/i', $subject, $match))
                return $match[2];
            //"Mad.Men.S06E11.HDTV.x264-2HD.par2" yEnc
            else if (preg_match('/^"(.+?)'.$this->e1, $subject, $match))
                return $match[1];
            //"Marvels.Agents.of.S.H.I.E.L.D.S01E07.HDTV.XviD-FUM.avi.nfo" [09/16] yEnc
                ...
                return $match[1];
            //"The Daily Show with Jon Stewart - March 2003.nzb" (1/1)
            else if (preg_match('/^"(The Daily Show.+)(\.nzb|\.part\d+\.rar)" \(\d+\/\d+\)/', $subject, $match))
                return $match[1];
            //[Deathy]-[U4A]-[Sugar Rush Season/Series 1 & 2] - "Sugar.Rush.S02E02.DVDRip.XviD.par2"
            else if (preg_match('/^\[Deathy\]-\[U4A\]-\[.+\][ -]+"(.+S\d+E\d+.+)\.par2"/', $subject, $match))
                return $match[1];
            //MP4SUX to the rescue [00/13] - "2.Broke.Girls.S02E09.And.The.New.Boss.HDTV.x264.LOL.mp4.nzb" yEnc
            else if (preg_match('/^MP4SUX to the rescue \[\d+\/\d+\] - "(.+)\.mp4\.nzb" yEnc$/', $subject, $match))
                return $match[1];
            //[Deadliest.Catch.S08E13.Landlocked.WEB-DL.x264-PTMiR] "Deadliest.Catch.S08E13.WEB-DL.x264-PTMiR.part29.rar" yEnc
            else if (preg_match('/^\[(.+S\d+E\d+.+)\] ".+" yEnc$/', $subject, $match))
                return $match[1];
            //[Zombieland.The.Series.S01E01.Pilot.720p.HDRiP.AC3-2.0.x264-AXED] - "Zombieland.The.Series.S01E01.r09" - 942.54 MB yEnc
            else if (preg_match('/^\[(.+S\d+E\d+.+)\][- ]+".+".+yEnc$/', $subject, $match))
                return $match[1];
            //[ 000000 ]-[FULL]-[#a.b.teevee@EFNet]-[ Law.And.Order.Criminal.Intent.S05 ] "Law.and.Order.CI.S05E19.Cruise.To.Nowhere.avi.par2" yEnc
            else if (preg_match('/\[[ ?\d#]+ ?\]-\[.+\]-\[.+\]-\[ (.+) \][ -]{0,3}("|#34;).+("|#34;) yEnc$/', $subject, $match))
                return $match[1];
            //Pass Free - "Game.of.Thrones.S03E02.720p.HDTV.x264-IMMERSE.part40.rar" yEnc
            else if (preg_match('/^Pass Free( RE-UPS)? - "(.+S\d+.+)(\.par2|\.part\d+\.rar|\.nfo)" yEnc$/i', $subject, $match))
                return $match[2];
            //Pass Free - "Game.of.Thrones.S03E02.720p.HDTV.x264-IMMERSE.rar" yEnc
            else if (preg_match('/^Pass Free( RE-UPS)? - "(.+S\d+.+)(\.par2|\.rar|\.nfo)" yEnc$/i', $subject, $match))
                return $match[2];
            //Derrick.DVDBOX1.S01E02.Johanna.German.FS.DVDRIP.XviD.NITRO         - (01/16) - "Derrick.DVDBOX1.S01E02.Johanna.nfo" [powered-by-usenet-4all.info] yEnc
            else if (preg_match('/^(.+s\d+e\d+.+NITRO)[\s-]+\(\d+\/\d+\)[ -]+".+" \[.+\] yEnc$/i', $subject, $match))
                return $match[1];
            //[FULL]Studio.60.on.the.Sunset.Strip.S01E04.The.West.Coast.Delay.720p.x264.5.1 [00/22] - "Studio.60.on.the.Sunset.Strip.S01E04.The.West.Coast.Delay.720p.x264.5.1.nzb" (1/1)
            else if (preg_match('/^\[FULL\](.+S\d+E\d+.+) \[\d+\/\d+\] - ".+" \(\d+\/\d+\)/i', $subject, $match))
                return $match[1];
            //[EyesOnly]-[TV]-[ Reba S02 DVD-TV Rip ]- "Reba S02E13.par2"
            else if (preg_match('/^\[EyesOnly\]-\[TV\]-\[ .+ \]- "(.+S\d+E\d+.+)\.par2"/i', $subject, $match))
                return $match[1];
            //(019/222) [Mad.Men.S01.480p.BluRay.X264-BoB]-[Mad Men Season 1] "Mad.Men.S01E02.480p.BluRay.X264-BoB.par2" - 169.82 kB - (Total - 2.41 GB) yEnc
            else if (preg_match('/^\(\d+\/\d+\) \[(.+S\d+.+)\]-\[.+\] ".+" .+ yEnc/i', $subject, $match))
                return $match[1];
            //Did You Hear About the Morgans XviD 720p - "Did.You.Hear.About.the.Morgans.par2"
            else if (preg_match('/^(.+Xvid (720|1080)p) - ".+"/i', $subject, $match))
                return $match[1];
            //NZBMatrix.com - "The X Factor S06E12 WS PDTV Xvid Tiggzz.par2" - 389.12 MB - yEnc
            else if (preg_match('/^NZBMatrix\.com - "(.+S\d+E\d+.+)\.par2" - .+ yEnc$/i', $subject, $match))
                return $match[1];
            //Arrow.S01E13.HDTV.x264.REPACK-LOL (not sure why its not being uploaded)  [00/21] - "Arrow.S01E13.HDTV.x264.REPACK-LOL.nzb"
            else if (preg_match('/^(.+S\d+E\d+.+LOL)( \(not.+\))?[ -]+\[\d+\/\d+\] - ".+"/i', $subject, $match))
                return $match[1];
            //Reno.911.S02.DVDRip.XviD-SAPHiRE - "Reno.911.S02.DVDRip.XviD-SAPHiRE.part24.rar" yEnc
            else if (preg_match('/^(.+S\d+.+) - ".+" yEnc$/i', $subject, $match))
                return $match[1];
            //<<<CowboyUp3101>>> usenet-space-cowboys.info <<<Powered by https://secretusenet.com>< "Derrick.S15E01.Fliegender.Vogel.GERMAN.FS.DVDRip.xviD_usenet-space-cowbys.info.avi" >< 03/12 (598,20 MB) >< 11,21 MB > yEnc
            else if (preg_match('/^<<<(Pitbull|Hustensaft\d+|Nimue|CowboyUp\d+(-\d+)?( Serie)?)>>> usenet-space-cowboys\.info <<<Powered by .+>< "(.+)\_usenet-space-cowbys\.info\.avi"/i', $subject, $match))
                return $match[4];
            //<<<Hustensaft0203HD>>><<<White.Collar.S01E02.Mord.ist.in.Mode.GERMAN.DUBBED.DL.1080p.BluRay.x264-TVP>>>usenet-space-cowboys.info<<<Powered by https://secretusenet.com>< "tvp-whitecollar-s01e02-1080p.r24" >< 27/87 (3,74 GB) >< 47,68 MB > yEnc
            else if (preg_match('/^<<<(Pitbull|Hustensaft\d+(HD)?|Nimue|CowboyUp\d+(-\d+)?( Serie)?)>>><<<(.+S\d+E\d+.+)>>>usenet-space-cowboys\.info<<<Powered.+ yEnc$/i', $subject, $match))
                return $match[5];
            //Got AVI XVID? [01/19] - "New.Tricks.S09E09.Part.of.a.Whole.HDTV.XVID.AVIGUY.avi.001" yEnc
            else if (preg_match('/^Got AVI XVID\? \[\d+\/\d+\] - "(.+)(\.AVIGUY| \(\d+ Showtime\)).+" yEnc$/i', $subject, $match))
                return $match[1];
            //[#a.b.teevee]-The.Listener.S01E10.720p.HDTV.x264-BiA "the.listener.s01e10.720p.hdtv.sample-bia.mkv.par2"  yEnc
            else if (preg_match('/^\[#?a\.b\.teevee\]-(.+S\d+E\d+.+BiA)\s+".+"\s+yEnc/i', $subject, $match))
                return $match[1];
            //National.Geographic.Great.Migrations.EP03.Race.to.Survive.2010.1080p.BluRay.x264.DTS-WiKi [00/49] - "National.Geographic.Great.Migrations.EP03.Race.to.Survive.2010.1080p.BluRay.x264.DTS-WiKi.nzb"
            else if (preg_match('/^(.+(EP\d+|\d{4}\.\d{2}\.\d{2}).+(WiKi|RUDOS))[\s-]+[\(\[]\d+\/\d+[\)\]\[\s-]+".+"/i', $subject, $match))
                return $match[1];
            //<<<usenet-space-cowboys.info>>><<<CowboyUp26>>><<<Powered by https://secretusenet.com>< "Torchwood.S03E01_usenet-space-cowbys.info.nfo" >< 02/11 (672,52 MB) >< 7,51 kB > yEnc
            else if (preg_match('/^<<<usenet-space-cowboys\.info>>><<<CowboyUp\d+>>><<<Powered.+>< "(.+S\d+E\d+.+)_usenet-space-cowbys\.info\.nfo".+yEnc$/i', $subject, $match))
                return $match[1];
            //<<<CowboyUp2012 Serien>>><<<The.Middle.S03E07.Das.zweite.Halloween.Fest.GERMAN.DUBBED.DL.720p.WebHD.h264-TVP>>>usenet-space-cowboys.info<<<Powered by https://secretusenet.com>< "tvp-themiddle-s03e07-720p.nfo.par2" >< 01/25 (865,47 MB) >< 8,72 kB > yEnc
            else if (preg_match('/^<<<(CowboyUp\s*\d+(-\d+)?( Filme| XXX| Doku| Serie(n)?)?)>>>\s*<<<(.+)>>>\s*usenet-space-cowboys\.info\s*<<<Powered/i', $subject, $match))
                return $match[5];
            //<<<CowboyUp26>>> <Powered by https://secretusenet.com> < "Torchwood.S01E02.par2" >< 01/16 (532,17 MB) >< 5,76 kB > yEnc
            else if (preg_match('/^<<<(CowboyUp\s*\d+(-\d+)?( Filme| XXX| Doku| Serie(n)?)?)>>>\s*<Powered by.+> < "(.+S\d+E\d+.+)\.par2" .+ yEnc$/i', $subject, $match))
                return $match[5];
            //<<<CowboyUp26>>> usenet-space-cowboys.info <<<Powered by https://secretusenet.com>< "Muppet Show e49-54_usenet-space-cowboys.info.par2" >< 01/23 (1,46 GB) >< 12,24 kB > yEnc
            else if (preg_match('/^<<<(CowboyUp\s*\d+(-\d+)?( Filme| XXX| Doku| Serie(n)?)?)>>>\s*usenet-space-cowboys\.info\s*<<<Powered by.+>< "(.+E\d+-\d+.+)_usenet-space-cowboys\.info\.par2" .+ yEnc$/i', $subject, $match))
                return $match[5];
            else
                return array("cleansubject" => $this->releaseCleanerHelper($subject), "properlynamed" => false);
        }
