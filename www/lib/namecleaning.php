...
class nameCleaning
{
	...
	public function collectionsCleaner($subject, $groupID="", $nofiles=false)
	{
		...
		else if ($groupName === "alt.binaries.highspeed")
		{
			...
			else if (preg_match('/^\(\?{4}\) \[\d+\/\d+\] - "(.+?)(\.part(\d+)?|\.rar|\.pdf)?(\.vol.+?"|\.[A-Za-z0-9]{2,4}"|") yEnc$/', $subject, $match))
				return $match[1];
			else
				return $this->collectionsCleanerHelper($subject, $groupName, $nofiles);
		}
	...
	public function releaseCleaner($subject, $groupID)
	{
		...
		else if ($groupName === "alt.binaries.classic.tv.shows")
		{
			...
			else if (preg_match('/^[a-zA-Z0-9][a-zA-Z0-9.-]+S\d+E\d+([a-zA-Z0-9.]+)? \[\d+\/\d+\] - "(.+?)(\.part(\d+)?|\.rar)?(\.vol.+?"|\.[A-Za-z0-9]{2,4}"|") yEnc$/', $subject, $match))
				return $match[2];
			//"Batman - S1E13-The Thirteenth Hat.par2" yEnc
			//"The Munsters - 1x01 Munster Masquerade.part.par" HQ DVDRip[02/16] yEnc
			else if (preg_match('/^(Re: )?"(.+?)(\.avi|\.mkv)?(\.part(\d+)?|\.rar)?(\.vol.+?"|\.[A-Za-z0-9]{2,4}"|")( HQ DVDRip\[\d+\/\d+\])? yEnc$/', $subject, $match))
				return $match[2];
			//Re: Outside Edge series 1 - [01/20] - "Outside Edge S01.nfo" yEnc
			//Green Acres Season 1 [01/87] - "Green Acres Season 1.par2" yEnc
			//MASH Season 1 - [01/54] - "MASH - Season 01.par2" yEnc
			else if (preg_match('/^(Re: )?[a-zA-Z0-9]+.+? (series|Season) \d+ (- )?\[\d+\/\d+\] - "(.+?)(\.part(\d+)?|\.rar)?(\.vol.+?"|\.[A-Za-z0-9]{2,4}"|") yEnc$/', $subject, $match))
				return $match[4];
			...
			else if (preg_match('/^(REQ - )?[A-Z0-9a-z][A-Z0-9a-z.]+ - \d+x\d+ (- [A-Z0-9a-z.]+ )?- \[\d+\/\d+\] - "(.+?)(\.avi|\.mkv)?(\.part(\d+)?|\.rar)?(\.vol.+?"|\.[A-Za-z0-9]{2,4}"|") yEnc$/', $subject, $match))
				return $match[3];
			else
				return $this->releaseCleanerHelper($subject);
		}
		...
		else if ($groupName === "alt.binaries.highspeed")
		{
			...
			else if (preg_match('/^\[\d+\/\d+\] - "(.+?) upp.by.+?(\.part(\d+)?|\.rar)?(\.vol.+?"|\.[A-Za-z0-9]{2,4}"|") yEnc$/', $subject, $match))
				return $match[1];
			...
			else if (preg_match('/^\[ .+? \] \[ www\..+? \]( \[.+?\])? \[\d+\/\d+\] - "(.+?)(\.part(\d+)?|\.rar)?(\.vol.+?"|\.[A-Za-z0-9]{2,4}"|") yEnc$/', $subject, $match))
				return $match[2];
			//< "Burn.Notice.S04E17.Out.of.the.Fire.GERMAN.DUBBED.DL.720p.WebHD.x264-TVP.par2" >< 01/17 (1.54 GB) >< 11.62 kB > yEnc
			else if (preg_match('/^< "(.+?)(\.part(\d+)?|\.rar)?(\.vol.+?"|\.[A-Za-z0-9]{2,4}"|") >< \d+\/\d+ \(.+?\) >< .+? > yEnc$/', $subject, $match))
				return $match[1];
			...
			else if (preg_match('/^\[\d+\/\d+ (.+?)(\.(part(\d+)?|rar|avi|iso|mp4|mkv|mpg))?(\.vol.+?"|\.[A-Za-z0-9]{2,4}"|") yEnc$/', $subject, $match))
			...
			else if (preg_match('/^"(.+?)(\.(part(\d+)?|rar|avi|mp4|mkv|mpg))?(\.vol.+?"|\.[A-Za-z0-9]{2,4}"|") yEnc$/', $subject, $match))
			{
				if (strlen($match[1]) > 7 && !preg_match('/\.vol.+/', $match[1]))
					return $match[1];
				else
