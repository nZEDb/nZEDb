<?php
	...
	class nameCleaning
	{
		...
		public function collectionsCleaner($subject, $type="normal")
		{
			...
			$cleansubject = preg_replace('/(\.part(\d{1,5})?)?\.(7z|\d{3}(?=(\s|"))|avi|epub|idx|iso|jpg|m4a|mds|mkv|mobi|mp4|nfo|nzb|pdf|rar|rev|rtf|r\d\d|sfv|srs|srr|sub|txt|vol.+(par2)|par(\s?2|")|zip|z{2,3})"?|(\s|(\d{2,3})?\-)\d{2,3}\.mp3|\d{2,3}\.pdf|\.part\d{1,4}\./i', ' ', $cleansubject);
			...
			//Random stuff.
			$cleansubject = utf8_encode(trim(preg_replace('/AutoRarPar\d{1,5}| \(\d+\)$|\d+Classic|part\d+/i', ' ', $cleansubject)));
			$cleansubject = trim(preg_replace('/\s\s+/i', ' ', $cleansubject));
