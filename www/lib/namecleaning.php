<?php
	...
	class nameCleaning
	{
		...
		public function collectionsCleaner($subject, $type="normal")
		{
			...
			$cleansubject = preg_replace('/( [a-z0-9]+-?|\(\d{4}\)_[a-z0-9]+)\.jpg|((\d{1,3})?\.part(\d{1,5})?|\d{1,5} ?|sample)?\.(7z|\d{3}(?=(\s|"))|avi|diz|epub|idx|iso|jpg|m3u|m4a|mds|mkv|mobi|mp4|nfo|nzb|par(\s?2|")|pdf|rar|rev|rtf|r\d\d|sfv|srs|srr|sub|txt|vol.+(par2)|xls|zip|z{2,3})"?|(\s|(\d{2,3})?\-)\d{2,3}\.mp3|\d{2,3}\.pdf|\.part\d{1,4}\./i', ' ', $cleansubject);
