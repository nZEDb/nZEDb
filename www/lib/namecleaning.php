<?php
	...
	class nameCleaning
	{
		...
		public function releaseCleaner($subject)
		{
			...
			//Books + Music.
			$cleanerName = preg_replace('/\[ATTN:.+?\]|ATTN: [a-z]{3,13} |ATTN:(macserv 100|Test)|ATTN: .+? - ("|:)|ATTN .+?:|[\.\-_ ]NMR \d{2,3}|by [a-z0-9]{3,15}$|enjoy!|\*enjoy\*|^ePub |New Ebooks \d{1,2} [a-z]{3,9} (19|20)\d\d|New Scans(,| )|Re: |Req: /i', '', $cleanerName);
			//Unwanted stuff.
			$cleanerName = preg_replace('/sampler$|\(\?\?\?\?\)|\[AoU\]|AsianDVDClub\.org|AutoRarPar\d{1,5}|brothers\-of\-usenet\.(info|net)(\/\.net)?|~bY ([a-z]{3,15}|c-w)|DVD-Freak|Ew-Free-Usenet-\d{1,5}|for\.usenet4ever\.info|ghost-of-usenet.org<<|GOU<<|(http:\/\/www\.)?friends-4u\.nl|\[\d+\]-\[abgxEFNET\]-|\[[a-z\d]+\]\-\[[a-z\d]+\]-\[FULL\]-|\[\d{3,}\]-\[FULL\]-\[(a\.b| abgx).+?\]|\[\d{1,}\]|\-\[FULL\].+?#a\.b[\w.#!@$%^&*\(\){}\|\\:"\';<>,?~` ]+\]|Lords-of-Usenet(\] <> presents)?|nzbcave\.co\.uk( VIP)?|Partner (of|von) SSL\-News\.info((>>)? presents)?|\/ post: |powere?d by (4ux(\.n\)?l)?|ssl-news(\.info)?|the usenet)|\-\s\[.+?\]\s<>\spresents|<.+?https:\/\/secretusenet\.com>|SECTIONED brings you|team-hush\.org\/| TiMnZb |<TOWN>|www\.binnfo\.in|www\.dreameplace\.biz|wwwworld\.me|(partner of )?www\.SSL-News\.info|www\.town\.ag|(Draak48|Egbert47|jipenjans|Taima) post voor u op|Sponsored\.by\.SecretUsenet\.com|(::::)?UR-powered by SecretUsenet.com(::::)?|usenet4ever\.info|(www\.)?usenet-4all\.info|www\.torentz\.3xforum\.ro|usenet\-space\-cowboys\.info|> USC <|SecretUsenet\.com|\] und \[|www\.[a-z0-9]+\.com|zoekt nog posters\/spotters|>> presents/i', '', $cleanerName);
