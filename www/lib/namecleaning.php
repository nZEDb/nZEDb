...
class nameCleaning
{
	...
	public function collectionsCleaner($subject, $groupID="", $nofiles=false)
	{
		...
		else if ($groupName === "alt.binaries.blu-ray")
		{
			...
			else if (preg_match('/^\(\d+(\/\d+\)(\s+ -)? "[a-zA-Z0-9]+\.).+?" - \d+[\.,]\d+ [kKmMgG][bB] - yEnc$/', $subject, $match))
				return $match[1];
			...
			else if (preg_match('/^([a-z0-9]+ \[.+?\] \[.+?\] \[)\d+,\d+ [kKmMgG][bB]\] \[\d+\/\d+\] ".+?" yEnc$/', $subject, $match))
				return $match[1];
		...
		else if ($groupName === "alt.binaries.boneless")
		{
			...
			else if (preg_match('/^\("([a-z0-9A-Z]+).+?" - \d+,\d+ [kKmMgG][bB] -\) ".+?" - \d+,\d+ [kKmMgG][bB] - yEnc$/', $subject, $match))
				return $match[1];
		...
		else if ($groupName === "alt.binaries.mojo")
		{
			...
			if (preg_match('/^\[\d+(\/\d+\] - ".+?)(\.part(\d+)?)?(\.vol.+?"|\.[A-Za-z0-9]{2,4}"|") - \d+[,\.]\d+ [kKmMgG][bB] - yEnc$/', $subject, $match))
				return $match[1];
			else
				return $this->collectionsCleanerHelper($subject, $nofiles);
		}
		...
		else if ($groupName === "alt.binaries.movies.divx")
		{
			...
			if (preg_match('/(.+?by mayhem masta".+?)(\.part(\d+)?)?(\.vol.+?"|\.[A-Za-z0-9]{2,4}"|") yEnc$/', $subject, $match))
				return $match[1];
			else
		...
		else if ($groupName === "alt.binaries.sounds.mp3")
		{
			...
			if (preg_match('/^(>>> CREATIVE COMMONS NZB <<< ".+?" - File )\d+ of \d+: ".+?" yEnc$/', $subject, $match))
				return $match[1];
			else
				return $this->collectionsCleanerHelper($subject, $nofiles);
		}
