...
class Music
{
	...
	public function processMusicReleases($threads=1)
	{
		...
		$ret = 0;
		$db = new DB();
		$res = $db->queryDirect(sprintf("SELECT searchname, ID from releases where musicinfoID IS NULL and nzbstatus = 1 and relnamestatus not in (0, 1) and categoryID in (3010, 3040, 3050) ORDER BY postdate desc LIMIT %d,%d", floor(max(0, $this->musicqty * $threads * 1.5)), $this->musicqty));
