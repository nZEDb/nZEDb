...
class Console
{
	...
	public function processConsoleReleases($threads=1)
	{
		...
		$this->processConsoleReleaseTypes($db->queryDirect(sprintf("SELECT searchname, ID from releases where consoleinfoID IS NULL and relnamestatus not in (0, 1) and nzbstatus = 1 and categoryID in ( select ID from category where parentID = %d ) ORDER BY postdate DESC LIMIT %d,%d", Category::CAT_PARENT_GAME, floor(($this->gameqty) * ($threads * 1.5)), $this->gameqty)), 1);
		// Names that were fixed and the release still doesn't have a consoleID.
		$this->processConsoleReleaseTypes($db->queryDirect(sprintf("SELECT searchname, ID from releases where consoleinfoID = -2 and relnamestatus not in (0, 1) and nzbstatus = 1 and categoryID in ( select ID from category where parentID = %d ) ORDER BY postdate DESC LIMIT %d,%d", Category::CAT_PARENT_GAME, floor(($this->gameqty) * ($threads * 1.5)), $this->gameqty)), 2);
	}
