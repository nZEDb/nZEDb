...
class Movie
{
	...
	public function processMovieReleases($releaseToWork = '')
	{
		...
		if ($releaseToWork == '')
		{
			$res = $this->db->query(sprintf("SELECT searchname AS name, id FROM releases WHERE imdbid IS NULL AND nzbstatus = 1 AND categoryid IN (SELECT id FROM category WHERE parentid = %d) LIMIT %d", Category::CAT_PARENT_MOVIE, $this->movieqty));
