...
class ReleaseCleaning
{
	...
	public function releaseCleaner($subject, $groupName, $usepre = false)
	{
		...
		switch ($groupName) {
			...
			case 'alt.binaries.multimedia.vintage-film.pre-1960':
				return multimedia_vintage_film_pre_1960();
			...
			case 'alt.binaries.astronomy':
				// add a case such that all groups handled by CollectionsCleaning are handled here, with generic
				return $this->generic();
	...
	{
		$cleanerName = preg_replace('/(- )?yEnc$/', '', $this->subject);
