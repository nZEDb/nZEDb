...
class Nfo 
{
	...
	public function processNfoFiles2($processImdb=1, $processTvrage=1)
	{
		$ret = 0;
		$db = new DB();
		$nntp = new Nntp();
		...
		$res = $db->queryDirect("SELECT rn.ID, r.guid, r.groupID, rn.releaseID FROM releasenfo rn left outer join releases r ON r.ID = rn.releaseID WHERE r.nfostatus between -5 and 0 order by adddate desc limit 0,50");
		if (mysql_num_rows($res) >= 0)
		{	
			if ($this->echooutput)
				echo "Processing ".mysql_num_rows($res)." NFO's.\n";
