...
class Nfo 
{
	...
	public function processNfoFiles($threads=0, $processImdb=1, $processTvrage=1)
	{
		...
		$res = $db->queryDirect(sprintf("SELECT ID, guid, groupID, name FROM releases WHERE nfostatus between -6 and -1 and nzbstatus = 1 order by adddate asc limit %d,%d", floor(($this->nzbs) * ($threads * 1.5)), $this->nzbs));
