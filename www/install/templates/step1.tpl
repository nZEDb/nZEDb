<table width="100%" border="0" style="margin-top:10px;" class="data highlight">
	...
	<tr class="">
		<td>Checking that audio covers dir is writeable:{if !$cfg->audioCoversCheck}<br /><span class="error">The covers/audio dir must be writable. A quick solution is to run:<br />chmod 777 {$cfg->WWW_DIR}/covers/audio</span>{/if}</td>
		<td>{if $cfg->audioCoversCheck}<span class="success">OK</span>{else}<span class="error">Error</span>{/if}</td>
	</tr>
	<tr class="alt">
		<td>Checking that audio sample dir is writeable:{if !$cfg->audiosampleCoversCheck}<br /><span class="error">The covers/audiosample dir must be writable. A quick solution is to run:<br />chmod 777 {$cfg->WWW_DIR}/covers/audiosample</span>{/if}</td>
		<td>{if $cfg->audiosampleCoversCheck}<span class="success">OK</span>{else}<span class="error">Error</span>{/if}</td>
	</tr>
	<tr class="">
		<td>Checking that book covers dir is writeable:{if !$cfg->bookCoversCheck}<br /><span class="error">The covers/book dir must be writable. A quick solution is to run:<br />chmod 777 {$cfg->WWW_DIR}/covers/book</span>{/if}</td>
		<td>{if $cfg->bookCoversCheck}<span class="success">OK</span>{else}<span class="error">Error</span>{/if}</td>
	</tr>
	<tr class="alt">
		<td>Checking that console covers dir is writeable:{if !$cfg->consoleCoversCheck}<br /><span class="error">The covers/console dir must be writable. A quick solution is to run:<br />chmod 777 {$cfg->WWW_DIR}/covers/console</span>{/if}</td>
	...
	<tr class="">
		<td>Checking that preview picture dir is writeable:{if !$cfg->previewCoversCheck}<br /><span class="error">The covers/preview dir must be writable. A quick solution is to run:<br />chmod 777 {$cfg->WWW_DIR}/covers/preview</span>{/if}</td>
		<td>{if $cfg->previewCoversCheck}<span class="success">OK</span>{else}<span class="error">Error</span>{/if}</td>
	</tr>
	<tr class="alt">
		<td>Checking that sample picture dir is writeable:{if !$cfg->sampleCoversCheck}<br /><span class="error">The covers/sample dir must be writable. A quick solution is to run:<br />chmod 777 {$cfg->WWW_DIR}/covers/sample</span>{/if}</td>
		<td>{if $cfg->sampleCoversCheck}<span class="success">OK</span>{else}<span class="error">Error</span>{/if}</td>
	</tr>
	<tr class="">
		<td>Checking that video sample dir is writeable:{if !$cfg->videoCoversCheck}<br /><span class="error">The covers/video dir must be writable. A quick solution is to run:<br />chmod 777 {$cfg->WWW_DIR}/covers/video</span>{/if}</td>
