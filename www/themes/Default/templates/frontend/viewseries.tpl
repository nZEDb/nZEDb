...
<div class="tvseriesheading">
	{if $show.image != 0}<img class="shadow" alt="{$show.title} Logo" src="{$smarty.const.WWW_TOP}/covers/tvshows/{$show.id}.jpg" />{/if}
	<p>
...
<div class="nzb_multi_operations">
	<div style="padding-bottom:10px;" >
		<a target="_blank" href="{$site->dereferrer_link}http://www.tvrage.com/shows/id-{$show.id}" title="View in TvRage">View in Tv Rage</a> |
		<a href="{$smarty.const.WWW_TOP}/rss?show={$show.id}{if $category != ''}&amp;t={$category}{/if}&amp;dl=1&amp;i={$userdata.id}&amp;r={$userdata.rsstoken}">Rss Feed for this Series</a>
	</div>
					...
					<td>
						...
						<div class="resextra">
							<div class="btns">
								...
								{if $result.firstdate != ""}<span class="rndbtn" title="{$result.title} Aired on {$result.firstdate|date_format}">Aired {if $result.firstdate|strtotime > $smarty.now}in future{else}{$result.firstdate|daysago}{/if}</span>{/if}
