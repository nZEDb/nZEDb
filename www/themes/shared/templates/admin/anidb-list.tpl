	...
	<table style="width:100%;margin-top:10px;" class="data Sortable highlight">
		...
		{foreach from=$anidblist item=anidb}
			<tr class="{cycle values=",alt"}">
				...
				<td width="84%"><a title="Edit" href="{$smarty.const.WWW_TOP}/anidb-edit.php?id={$anidb.anidbid}">{$anidb.title|escape:"htmlall"}</a></td>
				<td width="10%" class="center"><a title="Delete this AniDB entry" href="{$smarty.const.WWW_TOP}/anidb-delete.php?id={$anidb.anidbid}">delete</a> | <a title="Remove this anidbID from all releases" href="{$smarty.const.WWW_TOP}/anidb-remove.php?id={$anidb.anidbid}">remove</a></td>
			</tr>
		{/foreach}
	</table>
{else}
