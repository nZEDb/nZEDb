<div id="group_list"> 
    ...
    <table style="width:100%;" class="data highlight Sortable">
        ...
        <tr id="grouprow-{$group.id}" class="{cycle values=",alt"}">
            ...
            <td class="less" id="groupdel-{$group.id}"><a title="Reset and disable this group" href="javascript:ajax_group_reset({$group.id})" class="group_reset">Reset</a> | <a href="javascript:ajax_group_delete({$group.id})" class="group_delete" onclick="return confirm('Are you sure? This will delete the group from this list.');" >Delete</a> | <a href="javascript:ajax_group_purge({$group.id})" class="group_purge" onclick="return confirm('Are you sure? This will delete all releases, binaries/parts in the selected group');" >Purge</a></td>
        </tr>
        {/foreach}
