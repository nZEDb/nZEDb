	...
	<table style="width:100%;" class="data highlight icons" id="coverstable">
		...
		{foreach $data as $result}
			<tr class="{cycle values=",alt"}">
				<td class="mid">
					<div class="movcover">
						<img class="shadow" src="{$result->posters->{$site->rottentomatoquality}}" width="220" border="0" alt="{$result->title|escape:"htmlall"}" />
						<div class="movextra">
							...
							{if $cpurl != '' && $cpapi != ''}
								<a class="rndbtnsml sendtocouch" target="blackhole" href="javascript:;" rel="{$site->dereferrer_link}{$cpurl}/api/{$cpapi}/movie.add/?identifier=tt{$result->alternate_ids->imdb}&title={$result->title}" name="CP{$result->alternate_ids->imdb}" title="Add to CouchPotato">CouchPotato</a>
							{/if}
						</div>
					</div>
				</td>
