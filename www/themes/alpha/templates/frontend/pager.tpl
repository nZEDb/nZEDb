...
<li {if ($currentpage-1) < 1}class="disabled"{/if}>{if ($currentpage-1) < 1}<span><i class="icon-double-angle-left"></i> Prev</span>{else}<a href="{$pagerquerybase}{$pageroffset-$pageritemsperpage}{$pagerquerysuffix}"><i class="icon-double-angle-left"></i> Prev</a>{/if}</li>
{if $currentpage > 1}
<li><a href="{$pagerquerybase}0{$pagerquerysuffix}">1</a></li>{/if}

{if $currentpage > 3}<li class="disabled"><span>...</span></li>{/if}
...
{if ($currentpage+1) < ($pages-1) && ($currentpage+2) < $upperhalfwaypoint}<li class="disabled"><span>...</span></li>{/if}
...
{if ($upperhalfwaypoint+1) < $pages}<li class="disabled"><span>...</span></li>{/if}

{if $pages > $currentpage}<li><a href="{$pagerquerybase}{($pages*$pageritemsperpage)-$pageritemsperpage}{$pagerquerysuffix}">{$pages}</a></li>{/if}
<li{if ($currentpage+1) > $pages} class="disabled"{/if}>{if ($currentpage+1) > $pages}<span>Next <i class="icon-double-angle-right"></i></span>{else}<a href="{$pagerquerybase}{$pageroffset+$pageritemsperpage}{$pagerquerysuffix}">Next <i class="icon-double-angle-right"></i></a>{/if}</li>
</ul>
{/if}
