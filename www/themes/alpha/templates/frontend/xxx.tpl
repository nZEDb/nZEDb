			...
			<tr>
				<td style="vertical-align:top;text-align:center;width:150px;padding:10px;">
					<div class="movcover">
						<a
							target="_blank"
							href="{$smarty.const.WWW_TOP}/movies/?imdb={$result.id}"
							name="name{$result.id}"
				...
				<td colspan="3" class="left">
					<h2>
						<a
							...
							href="{$smarty.const.WWW_TOP}/xxx/?imdb={$result.imdbid}">{$result.title|stripslashes|escape:"htmlall"}
						</a>
						(<a
							class="title" title="{$result.year}"
							href="{$smarty.const.WWW_TOP}/xxx?year={$result.year}">{$result.year}
						</a>)
						{if $result.rating != ''}
							{$result.rating}/10
						{/if}
