	...
	<form id="nzb_multi_operations_form" action="get">
		...
		<table class="table table-striped table-condensed table-hover data icons" id="coverstable">
			...
			<tr>
				...
				<th>release date <a title="Sort Descending" href="{$orderbyreleasedate_desc}"><i
								class="icon-chevron-down"></i></a><a title="Sort Ascending"
																	 href="{$orderbyreleasedate_asc}"><i
								class="icon-chevron-up"></i></a></th>
						...
						<div class="movcover" style="padding-bottom:5px;">
							...
							<div class="movextra" style="margin-top:5px;">
								{if $result.nfoid > 0}<a href="{$smarty.const.WWW_TOP}/nfo/{$result.guid}"
										...
										Nfo</a> {/if}
								{if $result.url != ""}<a class="label" target="_blank"
														 href="{$site->dereferrer_link}{$result.url}"
					...
					<td colspan="8" class="left" id="guid{$result.guid}">
						...
						<div class="container">
							...
							{if $isadmin || $ismod}
								...
								<a class="label confirm_action label-danger"
...
{else}
	<div class="alert alert-link" style="vertical-align:middle;">
		...
		<div class="pull-left" style="margin-right: 15px;"><h2 style="margin-top: 7px;"> ಠ_ಠ </h2></div>
		<p>Either some amazon key is wrong, or there is nothing in this section.<br>Please try looking in the <a
					href="{$smarty.const.WWW_TOP}/browse?t={$category}"
					style="font-weight:strong;text-decoration:underline;">list view</a>.</p>
	</div>
{/if}