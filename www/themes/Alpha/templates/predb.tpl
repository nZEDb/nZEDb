...
<h4>{$page->title}</h4>
<form name="presearch" method="get" action="{$smarty.const.WWW_TOP}/predb" id="custom-search-form" class="form-inline form-horizontal col-4 col-lg-4 pull-right">
	...
	<table style="margin-bottom:10px; margin-top:5px;" class="table table-condensed table-highlight table-striped data Sortable">
		<thead>
			<tr>
				...
				<th style="width:60px;text-align:center;">ReqID</th>
				<th style="width:60px;text-align:center;">Size</th>
				<th style="width:60px;text-align:center;">Files</th>
		...
		{foreach $results as $result}
			<tr>
				...
				<td class="predb" style="text-align:center;">
					...
					{else}
						<span style="color:#{if $result.nuked == 1}009933{elseif $result.nuked > 1}990000{/if};" title="{if $result.nuked == 1}UNNUKED: {$result.nukereason|escape:"htmlall"}{elseif $result.nuked > 1}NUKED: {$result.nukereason|escape:"htmlall"}{else}{$result.title|escape:"htmlall"}{/if}">{$result.title|escape:"htmlall"|truncate:50}</span>
					{/if}
				</td>
