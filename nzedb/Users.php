...
class Users
{
	 ...
	 * @return bool|int
	 */
	public function add($userName, $firstName, $lastName, $password, $email, $role, $host, $invites = self::DEFAULT_INVITES, $invitedBy = 0)
	{
	...
	*/
	...
	public function getDownloadRequests($userID)
	{
		...
		$this->pdo->queryExec(
			sprintf(
				'DELETE FROM user_downloads WHERE user_id = %d AND timestamp < DATE_SUB(NOW(), INTERVAL 1 DAY)',
				$userID
			)
		);
		return $this->pdo->queryOneRow(
