...
class Console
{
	...
	function __construct($echooutput = false)
	{
		...
		$this->cleanconsole = ($site->lookupgames == 2) ? 4 : 0;
		$this->c = new ColorCLI();
	}
...
// Basically the XBLA names contain crap, this is to reduce the title down far enough to be usable.
	...
	{
		$db = $this->db;
		$res = $db->queryDirect(sprintf('SELECT r.searchname, r.id FROM releases r INNER JOIN category c ON r.categoryid = c.id WHERE nzbstatus = 1 AND (r.bitwise & %d) = %d AND r.consoleinfoid IS NULL AND c.parentid = %d ORDER BY r.postdate DESC LIMIT %d', $this->cleanconsole, $this->cleanconsole, Category::CAT_PARENT_GAME, $this->gameqty));
