...
class NameFixer
{
	...
	public function getPreFileNames(array $args = array())
	{
		...
		echo ColorCLI::primary("Matching predb filename to cleaned release_files.name.\n");
		...
		do {
			...
			$query = $this->pdo->queryDirect(
				sprintf("
					...
					GROUP BY r.id
					%s %s",
					($maxrelid !== null && $maxrelid > 0 ? "r.id > {$maxrelid} AND" : ''),
					$orderby,
					$limit
				)
			);
			...
			if ($query !== false) {
				...
				if ($total > 0 && $query instanceof \Traversable) {
					...
					foreach ($query as $row) {
						...
						if ($maxrelid !== null && $row['releases_id'] > $maxrelid) {
	...
	protected function _cleanMatchFiles()
	{
		...
		$this->_fileName = Text::stripNonPrintingChars($this->_fileName);
		...
		if (strlen($this->_fileName) > 0 && strpos($this->_fileName, '.') !== 0) {
			switch (true) {
				...
				case strpos($this->_fileName, '.') !== false:
					...
					$this->_fileName = Text::cutStringUsingLast('.', $this->_fileName, 'left', false);
					continue;
				...
				case strpos($this->_fileName, '\\') !== false:
					$this->_fileName = Text::cutStringUsingLast('\\', $this->_fileName, 'right', false);
					continue;
	...
	public function nfoCheckMus($release, $echo, $type, $namestatus, $show)
	{
		...
		if ($this->done === false && $this->relid !== $release['releases_id'] && preg_match('/(?:\s{2,})(.+?-FM-\d{2}-\d{2})/i', $release['textstring'], $result)) {
