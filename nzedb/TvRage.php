...
class TvRage
{
	...
	{
		return $this->pdo->queryOneRow(sprintf("SELECT * FROM tvrage_titles WHERE id = %d", $id));
	}
	...
	{
		return $this->pdo->query(sprintf("SELECT * FROM tvrage_titles WHERE rageid = %d", $id));
	}
	...
	public function getByTitle($title)
	{
		...
		if ($title != $title2) {
			...
			$res = $this->pdo->queryOneRow(sprintf("SELECT rageid FROM tvrage_titles WHERE replace(replace(releasetitle, %s, ''), '!', '') LIKE %s", $string, $this->pdo->escapeString($title4 . '%')));
			if (isset($res['rageid'])) {
				return $res['rageid'];
			}
		}
...
* @param $rageid
	...
	public function update($id, $rageid, $releasename, $desc, $genre, $country, $imgbytes)
	{
		...
		$this->pdo->queryExec(sprintf('UPDATE tvrage_titles SET rageid = %d, releasetitle = %s, description = %s, genre = %s, country = %s %s WHERE id = %d', $rageid, $this->pdo->escapeString($releasename), $this->pdo->escapeString(substr($desc, 0, 10000)), $this->pdo->escapeString($genre), $this->pdo->escapeString($country), $imgbytes, $id));
	}

	public function delete($id)
	{
		return $this->pdo->queryExec(sprintf("DELETE FROM tvrage_titles WHERE id = %d", $id));
	}
	...
	public function getRange($start, $num, $ragename = "")
	{
		...
		return $this->pdo->query(sprintf("SELECT id, rageid, releasetitle, description, createddate FROM tvrage_titles WHERE 1=1 %s ORDER BY rageid ASC" . $limit, $rsql));
	}
	...
	public function getCount($ragename = "")
	{
		...
		$res = $this->pdo->queryOneRow(sprintf("SELECT COUNT(id) AS num FROM tvrage_titles WHERE 1=1 %s", $rsql));
		return $res["num"];
	}
	...
	public function getCalendar($date = "")
	{
		...
		$sql = sprintf("SELECT * FROM tvrage_episodes WHERE DATE(airdate) = %s ORDER BY airdate ASC", $this->pdo->escapeString($date));
	...
	public function getSeriesList($uid, $letter = "", $ragename = "")
	{
		...
		return $this->pdo->query(
			sprintf("
				SELECT tvrage_titles.id, tvrage_titles.rageid, tvrage_titles.releasetitle, tvrage_titles.genre, tvrage_titles.country, tvrage_titles.createddate, tvrage_titles.prevdate, tvrage_titles.nextdate,
				...
				WHERE tvrage_titles.rageid IN (SELECT DISTINCT rageid FROM releases WHERE %s AND rageid > 0)
	...
	public function updateEpInfo($show, $relid)
	{
		...
		$this->pdo->queryExec(sprintf("UPDATE releases SET seriesfull = %s, season = %s, episode = %s, tvairdate = %s WHERE %s AND id = %d", $this->pdo->escapeString($show['seriesfull']), $this->pdo->escapeString($show['season']), $this->pdo->escapeString($show['episode']), $tvairdate, $this->catWhere, $relid));
	}
	...
	public function updateRageInfoTrakt($rageid, $show, $traktArray, $relid)
	{
		...
		if ($epinfo !== false) {
			$tvairdate = (!empty($epinfo['airdate'])) ? $this->pdo->escapeString($epinfo['airdate']) : "NULL";
			$tvtitle = (!empty($epinfo['title'])) ? $this->pdo->escapeString($epinfo['title']) : "NULL";
			$this->pdo->queryExec(sprintf("UPDATE releases SET tvtitle = %s, tvairdate = %s, rageid = %d WHERE %s AND id = %d", $this->pdo->escapeString(trim($tvtitle)), $tvairdate, $traktArray['ids']['tvrage'], $this->catWhere, $relid));
		} else {
			$this->pdo->queryExec(sprintf("UPDATE releases SET rageid = %d WHERE %s AND id = %d", $traktArray['ids']['tvrage'], $this->catWhere, $relid));
		}
		...
		foreach ($res as $arr) {
			...
			if (is_array($show) && $show['name'] != '') {
				...
				if ($id === false && $lookupTvRage) {
					...
					if ($tvairdate == "NULL") {
						$this->pdo->queryExec(sprintf('UPDATE releases SET tvtitle = %s, rageid = %d WHERE %s AND id = %d', $tvtitle, $id, $this->catWhere, $arr['id']));
					} else {
						$this->pdo->queryExec(sprintf('UPDATE releases SET tvtitle = %s, tvairdate = %s, rageid = %d WHERE %s AND id = %d', $tvtitle, $tvairdate, $id, $this->catWhere, $arr['id']));
					}
					// Cant find rageid, so set rageid to n/a.
				} else {
					$this->pdo->queryExec(sprintf('UPDATE releases SET rageid = -2 WHERE %s AND id = %d', $this->catWhere, $arr['id']));
				}
