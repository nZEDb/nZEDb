...
class ProcessReleases
{
	...
	public function processReleases($categorize, $postProcess, $groupName, &$nntp, $echooutput)
	{
		...
		if ($this->echoCLI) {
			$this->pdo->log::out(
				\sprintf('Starting release update process (%s)',
					date('Y-m-d H:i:s', $processReleases)),
				'header',
				true
			);
		}
		...
		if (!file_exists(Settings::value('..nzbpath'))) {
			if ($this->echoCLI) {
				$this->pdo->log::out('Bad or missing nzb directory - ' . Settings::value('..nzbpath'),
					'error',
					true
				);
			}
		...
		do {
			...
			if ($this->processRequestIDs === 0) {
				...
				$requestIDTime = time();
				if ($this->echoCLI) {
					$this->pdo->log::out('Process Releases -> Request ID Threaded lookup.', 'header');
				}
	...
	public function resetCategorize($where = '')
	{
		$this->pdo->queryExec(
			sprintf(/** @lang text */
				'UPDATE releases SET categories_id = %d, iscategorized = 0 %s', Category::OTHER_MISC, $where)
		);
	}
	...
	public function processIncompleteCollections($groupID)
	{
		...
		if ($this->echoCLI) {
			$this->pdo->log::out('Process Releases -> Attempting to find complete collections.', 'header', true);
		}
		...
		if ($this->echoCLI) {
			...
			$this->pdo->log::out(
				\sprintf('%s collections were found to be complete. Time: %s',
				\number_format($count === false ? 0 : $count['complete']),
	...
	public function processCollectionSizes($groupID)
	{
		...
		if ($this->echoCLI) {
			$this->pdo->log::out('Process Releases -> Calculating collection sizes (in bytes).', 'header', true);
		}
		...
		if ($checked !== false && $this->echoCLI) {
			$this->pdo->log::out(\sprintf('%s collections set to filecheck = 3(size calculated)',
				\number_format($checked->rowCount())),
				'primary',
				true
			);
			$this->pdo->log::out($this->consoleTools->convertTime(time() - $startTime),
				'primary',
				true
			);
		}
	...
	public function deleteUnwantedCollections($groupID)
	{
		...
		if ($this->echoCLI) {
			$this->pdo->log::out('Process Releases -> Delete collections smaller/larger than minimum size/file count from group/site setting.', 'header', true);
		}
		...
		if ($this->echoCLI) {
			$this->pdo->log::out(
				\sprintf("Deleted %s collections:\n%s smaller than, %s bigger than, %s with less files than site/group settings in: %s",
					\number_format($minSizeDeleted + $maxSizeDeleted + $minFilesDeleted),
					\number_format($minSizeDeleted),
					\number_format($maxSizeDeleted),
					\number_format($minFilesDeleted),
					$this->consoleTools->convertTime(time() - $startTime)
				),
				'primary',
				true
	...
	public function createReleases($groupID)
	{
		...
		if ($this->echoCLI) {
			$this->pdo->log::out("Process Releases -> Create releases from complete collections.", 'header', true);
		}
		...
		if ($this->echoCLI) {
			$this->pdo->log::out(
				\sprintf("\n%s Releases added and %s duplicate collections deleted in %s",
	...
	public function createNZBs($groupID)
	{
		...
		if ($this->echoCLI) {
			$this->pdo->log::out("Process Releases -> Create the NZB, delete collections/binaries/parts.", 'header', true);
		}
		...
		if ($this->echoCLI) {
			$this->pdo->log::out(
				\sprintf("%s NZBs created/Collections deleted in %s seconds.\nTotal time: %s",
					number_format($nzbCount),
					\number_format($totalTime),
					$this->consoleTools->convertTime($totalTime)
				),
				'primary',
				true
			);
	...
	public function processRequestIDs($groupID = '', $limit = 5000, $local = true)
	{
		...
		if ($this->echoCLI) {
			$this->pdo->log::out(
				sprintf('Process Releases -> Request ID %s lookup -- limit %s',
		...
		if ($this->echoCLI) {
			$this->pdo->log::out(
				\sprintf('%s releases updated in %s',
	...
	public function deleteCollections($groupID)
	{
		...
		if ($this->echoCLI) {
			$this->pdo->log::out(
				\sprintf("Finished deleting %s collections missed after NZB creation in %s seconds.\nRemoved %s parts/binaries/collection rows in %s",
					\number_format($deleted),
					\number_format(time() - $fourthQuery),
					number_format($deletedCount),
					$this->consoleTools->convertTime(($fourthQuery - $startTime))
				),
				'primary',
				true
			);
	...
	public function deletedReleasesByGroup($groupID = '')
	{
		...
		if ($this->echoCLI) {
			$this->pdo->log::out(
				\sprintf("Deleted %s releases: \n%s smaller than, %s bigger than, %s with less files than site/groups setting in: %s",
					\number_format($minSizeDeleted + $maxSizeDeleted + $minFilesDeleted),
					\number_format($minSizeDeleted),
					\number_format($maxSizeDeleted),
					\number_format($minFilesDeleted),
	...
	public function deleteReleases()
	{
		...
		if ($this->echoCLI) {
			$this->pdo->log::out('Process Releases -> Delete old releases and passworded releases.', 'header', true);
		}
		...
		if ($this->completion > 0) {
			$releases = $this->pdo->queryDirect(
				sprintf(/** @lang text */
					'SELECT SQL_NO_CACHE id, guid FROM releases WHERE completion < %d AND completion > 0', $this->completion)
			);
			if ($releases instanceof \Traversable) {
				foreach ($releases as $release) {
					$this->releases->deleteSingle(['g' => $release['guid'], 'i' => $release['id']], $this->nzb, $this->releaseImage);
		...
		if (count($disabledCategories) > 0) {
			foreach ($disabledCategories as $disabledCategory) {
				$releases = $this->pdo->queryDirect(
					sprintf(/** @lang text */
						'SELECT SQL_NO_CACHE id, guid FROM releases WHERE categories_id = %d', $disabledCategory['id'])
				);
				if ($releases instanceof \Traversable) {
					foreach ($releases as $release) {
		...
		if ($this->echoCLI) {
			$suffix = $this->completion > 0
				? ', ' . number_format($completionDeleted) . ' under ' . $this->completion . '% completion.'
				: '.';
			$this->pdo->log::out(
				\sprintf(
					'Removed releases: %s past retention, %s passworded, %s crossposted, %s' .
					' from disabled categories, %s smaller than category settings, %s' .
					' from disabled music genres, %s from misc->other %s from misc->hashed' . $suffix
					,
			...
			if ($totalDeleted > 0) {
				$this->pdo->log::out(
					\sprintf('Removed %s releases in %s',
	...
	private function processStuckCollections($where)
	{
		...
		if ($this->echoCLI && is_object($obj) && $obj->rowCount()) {
			$this->pdo->log::out('Deleted ' . $obj->rowCount() . ' broken/stuck collections.',
				'primary',
				true
			);
		}
