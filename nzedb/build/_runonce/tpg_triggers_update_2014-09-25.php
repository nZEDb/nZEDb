...
$tables = $pdo->queryDirect("SELECT SUBSTR(TABLE_NAME, 9) AS suffix, name FROM information_schema.TABLES WHERE TABLE_SCHEMA = (SELECT DATABASE()) AND TABLE_NAME LIKE 'binaries%' ORDER BY TABLE_NAME");
...
if ($tables instanceof \Traversable) {
	foreach ($tables as $table) {
		...
		$pdo->queryExec(sprintf($query1, $group['suffix']), true);
		$pdo->queryExec(sprintf($query2, $table['suffix']), true);
		$pdo->queryExec(sprintf($query3, $table['suffix'], $table['suffix'], $table['suffix'],
								$table['suffix']), true);
		$pdo->queryExec(sprintf($query4, $table['suffix'], $group['id']), true);
	}
