...
class TvRage
{
	...
	public function getByTitle($title)
	{
		...
		$string = ($this->pdo->dbSystem() === 'mysql' ? '"\'"' :  "E'\''");
	...
	public function add($rageid, $releasename, $desc, $genre, $country, $imgbytes)
	{
		...
		if ($this->pdo->dbSystem() === 'mysql') {
			...
			if (!isset($ckid['id'])) {
				$id = $this->pdo->queryInsert(sprintf('INSERT INTO tvrage (rageid, releasetitle, description, genre, country, createddate) VALUES (%d, %s, %s, %s, %s, NOW())', $rageid, $this->pdo->escapeString($releasename), $this->pdo->escapeString(substr($desc, 0, 10000)), $this->pdo->escapeString(substr($genre, 0, 64)), $this->pdo->escapeString($country)));
			} else {
				$id = $ckid['id'];
				$this->pdo->queryExec(sprintf('UPDATE tvrage SET releasetitle = %s, description = %s, genre = %s, country = %s, createddate = NOW() WHERE id = %d', $this->pdo->escapeString($releasename), $this->pdo->escapeString(substr($desc, 0, 10000)), $this->pdo->escapeString(substr($genre, 0, 64)), $this->pdo->escapeString($country), $id));
			}
			if ($imgbytes != '') {
	...
	public function update($id, $rageid, $releasename, $desc, $genre, $country, $imgbytes)
	{
		...
		if ($this->pdo->dbSystem() === 'mysql') {
			...
			$this->pdo->queryExec(sprintf('UPDATE tvrage SET rageid = %d, releasetitle = %s, description = %s, genre = %s, country = %s WHERE id = %d', $rageid, $this->pdo->escapeString($releasename), $this->pdo->escapeString(substr($desc, 0, 10000)), $this->pdo->escapeString($genre), $this->pdo->escapeString($country), $id));
			if ($imgbytes != '') {
				$path = nZEDb_COVERS . 'tvrage' . DS . $id . '.jpg';
				if (file_exists($path)) {
					unlink($path);
	...
	public function getRange($start, $num, $ragename = "")
	{
		...
		if ($ragename != "") {
			...
			$rsql .= sprintf("AND tvrage.releasetitle %s %s ", $like, $this->pdo->escapeString("%" . $ragename . "%"));
		}
	...
	public function getSeriesList($uid, $letter = "", $ragename = "")
	{
		...
		if ($letter != "") {
			...
			if ($this->pdo->dbSystem() === "mysql") {
				...
				$rsql .= sprintf("AND tvrage.releasetitle ~ %s", $this->pdo->escapeString('^' . $letter));
			}
		}
		$tsql = '';
		if ($ragename != '') {
	...
	public function updateSchedule()
	{
		...
		foreach ($countries as $country) {
			if ($this->echooutput) {
				echo $this->pdo->cli->headerOver('Updating schedule for: ') . $this->pdo->cli->primary($country['country']);
			}
			...
			if ($sched !== false && ($xml = @simplexml_load_string($sched))) {
						...
						foreach ($sTime as $sShow) {
							...
							if (in_array($currShowId, $showarray)) {
								if ($this->pdo->dbSystem() === 'mysql') {
									...
									$check = $this->pdo->queryOneRow(sprintf('SELECT id FROM tvrageepisodes WHERE rageid = %d', $sShow->sid));
									if ($check === false) {
										$this->pdo->queryExec(sprintf("INSERT INTO tvrageepisodes (rageid, showtitle, fullep, airdate, link, eptitle) VALUES (%d, %s, %s, %s, %s, %s)", $sShow->sid, $this->pdo->escapeString($currShowName), $this->pdo->escapeString($sShow->ep), $this->pdo->escapeString(date("Y-m-d H:i:s", $day_time)), $this->pdo->escapeString($sShow->link), $this->pdo->escapeString($sShow->title)));
									} else {
										$this->pdo->queryExec(sprintf('UPDATE tvrageepisodes SET showtitle = %s, fullep = %s, airdate = %s, link = %s, eptitle = %s WHERE id = %d', $this->pdo->escapeString($currShowName), $this->pdo->escapeString($sShow->ep), $this->pdo->escapeString(date("Y-m-d H:i:s", $day_time)), $this->pdo->escapeString($sShow->link), $this->pdo->escapeString($sShow->title), $check['id']));
									}
								}
							}
						}
				...
				foreach ($xmlSchedule as $showId => $epInfo) {
					...
					if (sizeof($res) > 0) {
						foreach ($res as $arr) {
							...
							// Output.
							if ($this->echooutput) {
								echo $this->pdo->cli->primary($epInfo['showname'] . " (" . $showId . "):");
								if (isset($epInfo['prev']['day_time'])) {
									echo $this->pdo->cli->headerOver("Prev EP: ") . $this->pdo->cli->primary("{$prev_ep} - " . date("m/d/Y H:i T", $epInfo['prev']['day_time']));
								}
								if (isset($epInfo['next']['day_time'])) {
									echo $this->pdo->cli->headerOver("Next EP: ") . $this->pdo->cli->primary("{$next_ep} - " . date("m/d/Y H:i T", $epInfo['next']['day_time']));
								}
								echo "\n";
							}
				...
				if ($this->echooutput) {
					echo $this->pdo->cli->info("Schedule not found.");
				}
			}
		}
		if ($this->echooutput) {
			echo $this->pdo->cli->primary("Updated the TVRage schedule succesfully.");
		}
	}
	...
	{
		if ($this->echooutput) {
			echo $this->pdo->cli->headerOver("Updating Episode: ") . $this->pdo->cli->primary($show['cleanname'] . " " . $show['seriesfull'] . (($show['year'] != '') ? ' ' . $show['year'] : '') . (($show['country'] != '') ? ' [' . $show['country'] . ']' : ''));
		}
	...
	public function processTvReleases($groupID = '', $guidChar = '', $lookupTvRage = 1, $local = false)
	{
		...
		if ($this->echooutput && $tvcount > 1) {
			echo $this->pdo->cli->header("Processing TV for " . $tvcount . " release(s).");
		}
		...
		foreach ($res as $arr) {
			...
			if (is_array($show) && $show['name'] != '') {
				...
				if ($id === false && $lookupTvRage) {
					...
					if ($this->echooutput) {
						echo $this->pdo->cli->primaryOver("TVRage ID for ") . $this->pdo->cli->headerOver($show['cleanname']) . $this->pdo->cli->primary(" not found in local db, checking web.");
					}
					...
					if ($tvrShow !== false && is_array($tvrShow)) {
						...
						if ($traktArray !== false) {
							if (isset($traktArray['show']['tvrage_id']) && $traktArray['show']['tvrage_id'] !== 0) {
								if ($this->echooutput) {
									echo $this->pdo->cli->primary('Found TVRage ID on trakt:' . $traktArray['show']['tvrage_id']);
								}
					...
					//if ($this->echooutput) {
					//    echo $this->pdo->cli->AlternateOver("TV series: ") . $this->pdo->cli->header($show['cleanname'] . " " . $show['seriesfull'] . (($show['year'] != '') ? ' ' . $show['year'] : '') . (($show['country'] != '') ? ' [' . $show['country'] . ']' : ''));
					// }
					$tvairdate = (isset($show['airdate']) && !empty($show['airdate'])) ? $this->pdo->escapeString($this->checkDate($show['airdate'])) : "NULL";
					$tvtitle = "NULL";
	...
	public function getRageMatch($showInfo)
	{
		...
		if ($xml !== false) {
			...
			if (isset($arrXml['show']) && is_array($arrXml)) {
				...
				if (isset($titleMatches[100])) {
					if ($this->echooutput) {
						echo $this->pdo->cli->primary('Found 100% match: "' . $titleMatches[100][0]['title'] . '"');
					}
				...
				if (isset($urlMatches[100])) {
					if ($this->echooutput) {
						echo $this->pdo->cli->primary('Found 100% url match: "' . $urlMatches[100][0]['title'] . '"');
					}
				...
				if (isset($akaMatches[100])) {
					if ($this->echooutput) {
						echo $this->pdo->cli->primary('Found 100% aka match: "' . $akaMatches[100][0]['title'] . '"');
					}
				...
				foreach ($titleMatches as $mk => $mv) {
					...
					if ($this->echooutput) {
						echo $this->pdo->cli->primary('Found ' . $mk . '% match: "' . $titleMatches[$mk][0]['title'] . '"');
					}
				...
				foreach ($akaMatches as $ak => $av) {
					...
					if ($this->echooutput) {
						echo $this->pdo->cli->primary('Found ' . $ak . '% aka match: "' . $akaMatches[$ak][0]['title'] . '"');
					}
					return $akaMatches[$ak][0];
				}

				if ($this->echooutput) {
					echo $this->pdo->cli->primary('No match found on TVRage trying Trakt.');
				}
				return false;
			} else {
				if ($this->echooutput) {
					echo $this->pdo->cli->primary('Nothing returned from tvrage.');
				}
				return false;
			}
		} else {
		...
		if ($this->echooutput) {
			echo $this->pdo->cli->primary('No match found online.');
		}
		return false;
	}
