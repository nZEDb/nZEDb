...
class Groups
{
	...
	public function reset($id)
	{
		...
		$cols = $this->pdo->query(sprintf("SELECT id FROM collections WHERE group_id = %d", $id));
		foreach ($cols as $col) {
			$binaries->delete($col["id"]);
		}
		$this->pdo->queryExec(sprintf("DELETE FROM partrepair WHERE group_id = %d", $id));
		$this->pdo->queryExec('DROP TABLE IF EXISTS collections_'.$id);
		$this->pdo->queryExec('DROP TABLE IF EXISTS binaries_'.$id);
		$this->pdo->queryExec('DROP TABLE IF EXISTS parts_'.$id);
		$this->pdo->queryExec('DROP TABLE IF EXISTS partrepair_'.$id);
