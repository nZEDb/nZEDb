...
Class Greenlight
{
	...
	public function trailer()
	{
		if (isset($this->response) && isset($this->title)) {
			...
			if(preg_match('/(?<videourl>src\=\"(.*)movie480.webm\?(.*)\")/', $this->response,$matches)){
				print_r($matches); exit;
				$this->res['trailer'] = self::CDNURL . $this->steamgameid . '/' . trim($matches['videourl'],'"');
			}
			}
		}
	...
	public function search()
	{
		...
		if ($this->_geturl(self::GREENLIGHTURL . '?searchtext=' . urlencode($this->searchterm) . self::GREENLIGHTVARS ) !== false) {
			print_r($this->response); exit;
			if ($ret = $this->html->find("div.workshopItemTitle", 0)) {
 ...
 */
	...
	private function _geturl($fetchurl = null, $usepost = false)
	{
		...
		curl_setopt($this->ch, CURLOPT_REFERER, self::GREENLIGHTURL);
