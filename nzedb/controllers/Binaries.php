...
class Binaries
{
							...
							if ($collectionCheck === false) {
								$collectionID = $this->_pdo->queryInsert(
									sprintf("
										...
										$groupNames['cname'],
										$this->_pdo->escapeString(substr($subject, 0, 255)),
						...
						if ($binaryCheck === false) {
							$binaryID = $this->_pdo->queryInsert(
								sprintf("
									...
									$groupNames['bname'],
									$binaryHash,
									$this->_pdo->escapeString($subject),
									$collectionID,
									$data['MaxParts'],
									$data['File']
								)
	...
	{
		$bins = $this->_pdo->query(sprintf('SELECT id FROM binaries WHERE collectionid = %d', $id));
		foreach ($bins as $bin) {
			$this->_pdo->queryExec(sprintf('DELETE FROM parts WHERE binaryid = %d', $bin['id']));
		}
		$this->_pdo->queryExec(sprintf('DELETE FROM binaries WHERE collectionid = %d', $id));
		$this->_pdo->queryExec(sprintf('DELETE FROM collections WHERE id = %d', $id));
	}
