...
class ReleaseCleaning
{
	...
	public function releaseCleaner($subject, $fromName, $size, $groupName, $usepre = false)
	{
		...
		if (preg_match('/^(\[ ?(?P<req>\d+) ?\]|REQ\s*(?P<req>\d+)|\d{4,6}-\d)/i', $this->subject, $match)) {
	...
	public function _3d()
	{
		...
		if (preg_match('/^"(.+?)' . $this->e2, $this->subject, $match)) {
			return $match[1];
		}
		return array("cleansubject" => $this->releaseCleanerHelper($this->subject), "properlynamed" => false);
	}
	...
	public function ath()
	{
		...
		if (preg_match('/^<Have Fun> "(.+?)' . $this->e0 . ' SpongeBoZZ yEnc$/', $this->subject, $match)) {
	...
	public function barbarella()
	{
		...
		if (preg_match('/^.+?\s{2,}\d+[,.]\d+ [mMkKgG][bB]\s{2,}"(.+?)' . $this->e0 . '\s{2,}(\d+ B|\d+[,.]\d+ [mMkKgG][bB]) yEnc$/', $this->subject, $match)) {
			return $match[1];
	...
	public function blu_ray()
	{
		...
		if (preg_match(
			'/^[\w-\. ]+[ _-]{0,3}[\(\[]\d+\/\d+[\)\]][ _-]{0,3}"(.+?)' . $this->e1, $this->subject,
			$match
		)
		) {
			return $match[1];
		...
		if (preg_match('/^[\w-\. ]+[ _-]{0,3}[\(\[]\d+\/\d+[\)\]][ _-]{0,3}"(.+?)' . $this->e2, $this->subject, $match)) {
			return $match[1];
		}
		return array("cleansubject" => $this->releaseCleanerHelper($this->subject), "properlynamed" => false);
	}
	...
	public function boneless()
	{
		...
		if (preg_match('/^\(aangemeld bij usenet collector\) \[\d+\/\d+\] - "(.+?)' . $this->e1, $this->subject, $match)) {
			return $match[1];
		...
		if (preg_match('/^\[\d+\/\d+\] - "(.+?)' . $this->e0 . ' - \d+[.,]\d+ [kKmMgG][bB] .+usenet-space-cowboys\.info.+ yEnc$/', $this->subject, $match)) {
			return $match[1];
		...
		if (preg_match('/^brothers-of-usenet\.info.+ - \[\d+\/\d+\] - "(.+?)' . $this->e1, $this->subject, $match)) {
			return $match[1];
	...
	public function british_drama()
	{
		...
		if (preg_match('/^.+?[-_ ]{0,3}\d+[xX]\d+[-_ ]{0,3}.+?\[\d+\/\d+\][-_ ]{0,3}.+?"(.+?)' . $this->e1, $this->subject, $match)) {
			return $match[1];
		...
		if (preg_match('/^(.+ S\d+E\d+)[-_ ]{0,3}\[\d+\/\d+\][-_ ]{0,3}"(.+?)' . $this->e1, $this->subject, $match)) {
			return $match[1];
	...
	public function cbt()
	{
		...
		if (preg_match('/^"(.+?)' . $this->e0 . ' {1,2}yEnc$/', $this->subject, $match)) {
			return $match[1];
		...
		if (preg_match('/^\(\?+\) \[\d+\/\d+\] - "(.+?)' . $this->e0 . ' Sayco  yEnc$/', $this->subject, $match)) {
			return $match[1];
		}
		return array("cleansubject" => $this->releaseCleanerHelper($this->subject), "properlynamed" => false);
	}
	...
	public function classic_tv_shows()
	{
		...
		if (preg_match('/^[a-zA-Z0-9][a-zA-Z0-9.-]+S\d+E\d+([a-zA-Z0-9.]+)? \[\d+\/\d+\] - "(.+?)' . $this->e1, $this->subject, $match)) {
			return $match[2];
		}
		...
		if (preg_match('/^(Re: )?[a-zA-Z0-9]+.+? (series|Season) \d+ (- )?\[\d+\/\d+\] - "(.+?)' . $this->e1, $this->subject, $match)) {
			return $match[4];
		}
	...
	public function cores()
	{
		...
		if (preg_match('/^.+Usejunkies.tk.+ \[\d+\/\d+\] - "(.+?)' . $this->e2, $this->subject, $match)) {
			return $match[1];
		} //(5/9) :...Usejunkies.tk...:----"Leawo.Blu-ray.Ripper.v4.4.0.rar.vol003+04.par2" - 30.94 MB - yEnc
		if (preg_match('/^\(\d+\/\d+\).+Usejunkies.tk.+"(.+?)' . $this->e2, $this->subject, $match)) {
			return $match[1];
		} //(Grimms03e10) [6/9] - "Grimm_S03E10_Eyes_of_the_Beholder.avi.vol007+08.PAR2"Grimm S03E10  yEnc
		if (preg_match('/^\(.+s\d+e\d+\) \[\d+\/\d+\] - "(.+?)' . $this->e0 . '.+yEnc$/', $this->subject, $match)) {
			return $match[1];
		}
	...
	public function documentaries()
	{
		...
		if (preg_match('/^Battlefield S\d S\d+E\d+ "(.+?)' . $this->e1, $this->subject, $match)) {
			return $match[1];
		...
		if (preg_match('/^.+? S\d+E\d+ - "(.+?)' . $this->e0 . '  (\d+[,.]\d+ [kKmMgG][bB]ytes) yEnc$/', $this->subject, $match)) {
			return $match[1];
		...
		if (preg_match('/^"(.+?)' . $this->e0 . ' -  \[\d+ of \d+ - \d+([,.]\d+)? [kKmMgG][bB]\] yEnc$/', $this->subject, $match)) {
			return $match[1];
		...
		if (preg_match('/^[\w ]+"(.+?)' . $this->e0 . '(.+[\(\[]\d+\/\d+[\)\]])?[- ]{0,3}yEnc$/', $this->subject, $match)) {
			return $match[1];
		} //[Walmart - The High Cost of Low Prices]-[28/59] - "wthcolp.part26.rar" yEnc
		if (preg_match('/^\[([\w- ]+)\]-\[\d+\/\d+\] - "(.+?)' . $this->e1, $this->subject, $match)) {
			return $match[1];
		...
		if (preg_match('/^\(World Air Routes.+\) \[\d+\/\d+\] - "(.+?)' . $this->e1, $this->subject, $match)) {
			return $match[1];
		...
		if (preg_match('/^dokus4alle presents \(\d+\/\d+\) "(.+?)' . $this->e2, $this->subject, $match)) {
			return $match[1];
	...
	public function ebook()
	{
		...
		if (preg_match('/(.+)[-_ ]{0,3}[\(\[]\d+\/\d+[\)\]][-_ ]{0,3}"(.+?)' . $this->e0 . '([-_ ]{0,3}yEnc){1,2}$/i', $this->subject, $match)) {
			return $match[2];
	...
	public function erotica()
	{
		...
		if (preg_match('/^\[\d+\/\d+\][- ]{0,3}"([\w.()-]{8,}?)' . $this->e1, $this->subject, $match)) {
			return $match[1];
		} //>>>>>Hell-of-Usenet.org>>>>> - [01/23] - "Cum Hunters 3 XXX.par2" yEnc
		if (preg_match('/^[><]+Hell-of-Usenet\.org[<>]+ - \[\d+\/\d+\] - "(.+?)' . $this->e1, $this->subject, $match)) {
			return $match[1];
		} //Lesbian Crush Diaries 5 XXX DVDRip x264-Pr0nStarS - (01/26) "Lesbian.Crush.Diaries.5.XXX.DVDRip.x264-Pr0nStarS.nfo" - yenc
		if (preg_match('/^[A-Z0-9][a-zA-Z0-9 -]{6,}? - \(\d+\/\d+\) "(.+?)' . $this->e0 . ' - yenc yEnc$/', $this->subject, $match)) {
			return $match[1];
	...
	public function etc()
	{
		...
		if (preg_match('/^.+\[DoAsYouLike\].+\d+[.,]\d+ [kKmMgG][bB].+"(.+?)' . $this->e0 . '    (\d+[.,])?\d+ ([kKmMgG])?[bB][- ]{0,3}yEnc$/', $this->subject, $match)) {
			return $match[1];
		...
		if (preg_match('/^.+Old but Sold.+>< "(.+?)' . $this->e0 . ' >< \d+\/\d+ \(\d+[.,]\d+ [kKmMgG][bB]\).+ yEnc$/', $this->subject, $match)) {
			return $match[1];
		...
		if (preg_match('/^.+?usenet-space.+?Powered by.+? "([\w. ()-]{8,}.?)' . $this->e0 . '.+? \d+\/(\d+.+?)$/', $this->subject, $match)) {
			return $match[1];
		} //<<<< old but sold >>>> < USC> <"K11 - Kommissare im Einsatz (DE) NDS.part3.rar">[04/11] 74,54 MB yEnc
		if (preg_match('/^.+old but sold.+"([\w. ()-]{8,}.?)' . $this->e0 . '>\[\d+\/\d+\][- ]{0,3}\d+[.,]\d+ [kKmMgG][bB][- ]{0,3}yEnc$/', $this->subject, $match)) {
			return $match[1];
		...
		if (preg_match('/^[\w. -]+[- ]{0,3}"([\w.()-]{8,}.?)' . $this->e1, $this->subject, $match)) {
			return $match[1];
		} //Man.Of.Steel.2013.BRRip.x264.AC3-UNiQUE - [02/38] - "Man.Of.Steel.2013.BRRip.x264.AC3-UNiQUE.part01.rar" yEnc
		if (preg_match('/^[\w. -]+[- ]{0,3}\[\d+\/\d+\][- ]{0,3}"([\w.()-]{8,}.?)' . $this->e1, $this->subject, $match)) {
			return $match[1];
		} //>GOU<< XUS Clock Plus v1.5.0 with Key [TorDigger].PAR2 >>www.SSL-News.info<  yEnc
		if (preg_match('/^>GOU<< ([\w.\[\]() -]{8,}.?)([-_](proof|sample|thumbs?))*(\.part\d*(\.rar)?|\.rarr|\.7z)?(\d{1,3}\.rev|\.vol.+?|\.[A-Za-z0-9]{2,4}).+www\.SSL-News\.info<[- ]{0,3}yEnc$/', $this->subject, $match)) {
			return $match[1];
		} //Man.Of.Steel.2013.BRRip.x264.AC3-UNiQUE - [02/38] - "Man.Of.Steel.2013.BRRip.x264.AC3-UNiQUE.part01.rar" yEnc
		if (preg_match('/^[\w. -]+[- ]{0,3}\[\d+\/\d+\][- ]{0,3}"([\w.()-]{8,}.?)' . $this->e1, $this->subject, $match)) {
			return $match[1];
		...
		if (preg_match('/^"([\w.()-]{8,}?)' . $this->e1, $this->subject, $match)) {
			return $match[1];
		}
		return array("cleansubject" => $this->releaseCleanerHelper($this->subject), "properlynamed" => false);
	}
	...
	public function ftn()
	{
		...
		if (preg_match('/^(Usenet collector)?\(aangemeld.+\) \[\d+\/\d+\] - "(.+?)' . $this->e1, $this->subject, $match)) {
			return $match[2];
	...
	public function games_dox()
	{
		...
		if (preg_match('/^\[NEW DOX\][ _-]{0,3}(.+?)[ _-]{0,3}\[\d+\/\d+\][ _-]{0,3}"(.+?)' . $this->e1, $this->subject, $match)) {
			return $match[1];
		} //[NEW DOX] Minecraft.1.6.2.Installer.Updated.Server.List  - "Minecraft 1 6 2 Cracked Installer Updater Serverlist.nfo" - yEnc
		if (preg_match('/^\[NEW DOX\][ _-]{0,3}(.+?)[ _-]{0,3}"(.+?)' . $this->e1, $this->subject, $match)) {
			return $match[1];
	...
	public function hdtv()
	{
		...
		if (preg_match('/\(Newseros\.com\) .+? \[\d+\/\d+\] - "([\w.()-]{8,}?)' . $this->e1, $this->subject, $match)) {
			return $match[1];
		...
		if (preg_match('/^.+S\d+E\d+.+\[\d+\/\d+\] - "(.+?)' . $this->e1, $this->subject, $match)) {
			return $match[1];
		...
		if (preg_match('/^\[ TrollHD \] - [\(\[] \d+\/\d+ [\)\]] - "(.+?)' . $this->e1, $this->subject, $match)) {
			return $match[1];
		} //Adobe Photoshop Lightroom v5.2 - FiNaL - Multilingual [WIN].vol037+32.PAR2  yEnc
		if (preg_match('/^(.+?)([-_](proof|sample|thumbs?))*(\.part\d*(\.rar)?|\.rar)?(\d{1,3}\.rev|\.vol.+?|\.[A-Za-z0-9]{2,4}) {0,3}yEnc$/', $this->subject, $match)) {
			return $match[1];
	...
	public function hdtv_x264()
	{
		...
		if (preg_match('/^(AMEa)?\(\d+\/\d+\)( - AME-)? "(.+?)' . $this->e0 . ' - \d+[,.]\d+ [mMkKgG][bB] - yEnc$/', $this->subject, $match)) {
			return $match[3];
		}
		...
		if (preg_match('/^[A-Z][a-zA-Z0-9 ]+ [- ] "(.+?)' . $this->e1, $this->subject, $match)) {
			return $match[1];
	...
	public function highspeed()
	{
		...
		if (preg_match('/^\[ .+? \] \[ www\..+? \]( \[.+?\])? \[\d+\/\d+\] - "(.+?)' . $this->e1, $this->subject, $match)) {
			return $match[2];
	...
	public function inner_sanctum()
	{
		...
		if (preg_match('/^[a-zA-Z0-9._-]+ \[\d+\/\d+\] - "(.+?)' . $this->e1, $this->subject, $match) && !preg_match('/[a-fA-F0-9]{32}/', $this->subject)) {
			return $match[1];
		}
	...
	public function mom()
	{
		...
		if (preg_match('/^\[Art-of-Usenet\] .+? \[\d+\/\d+\][-_ ]{0,3}"(.+?)' . $this->e1, $this->subject, $match)) {
			return $match[1];
		} //<ghost-of-usenet.org>XCOM.Enemy.Unknown.Deutsch.Patch.TokZic [0/9] - "XCOM Deutsch.nzb" ein CrazyUpp yEnc
		if (preg_match('/^<ghost-of-usenet\.org>(.+?) \[\d+\/\d+\] - ".+?" .+? yEnc$/', $this->subject, $match)) {
			return $match[1];
		...
		if (preg_match('/brothers-of-usenet\.info&net-empfehlen-ssl-news.+\[\d+\/\d+\] - "(.+?)' . $this->e1, $this->subject, $match)) {
			return $match[1];
		} //Eragon postet   Pfeil   S01E18   german                      [01/19] - "Pfeil S01E18.par2" yEnc
		if (preg_match('/^Eragon postet.+\[\d+\/\d+\] - "(.+?)' . $this->e1, $this->subject, $match)) {
			return $match[1];
	...
	public function movies()
	{
		...
		if (preg_match('/^\(\d+\/\d+\)[- ]{0,3}([\w.() -]{8,}.?)[- ]{0,3}".+?' . $this->e2, $this->subject, $match)) {
			return $match[1];
	...
	public function movies_x264()
	{
		...
		if (preg_match('/^\([\w.-]+\)[\(\[]\d+\/\d+[\]\)] "(.+?)' . $this->e2, $this->subject, $match)) {
			return $match[1];
		...
		if (preg_match('/^"(.+?)' . $this->e0 . ' \d+ of \d+[- ]{0,3}yEnc$/', $this->subject, $match)) {
			return $match[1];
	...
	public function mp3()
	{
		...
		if (preg_match('/^(.+?)[-_ ]{0,3}("|#34;)(.+?)' . $this->e1, $this->subject, $match)) {
			return $match[3];
		}
		return array("cleansubject" => $this->releaseCleanerHelper($this->subject), "properlynamed" => false);
	}
	...
	public function mp3_complete_cd()
	{
		...
		if (preg_match('/^<www\.lords-of-usenet\.org><by Nerts>[ _-]{0,3}\(\d+\/\d+\)[ _-]{0,3}[ _-]{0,3}"(.+?)' . $this->e0 . '[ _-]{0,3}\d+[.,]\d+ [kKmMgG][bB][ _-]{0,3}yEnc$/i', $this->subject, $match)) {
			return $match[1];
		...
		if (preg_match('/^\[\d+\/\d+\] - \((MP3 )?ALBUM CHARTS\) - "(.+?)' . $this->e0 . '  yEnc$/i', $this->subject, $match)) {
			return $match[2];
		} //"Miley Cyrus - Bangerz (Deluxe Version).nzb" - yEnc
		if (preg_match('/^"(.+?)' . $this->e0 . ' - yEnc$/i', $this->subject, $match)) {
			return $match[1];
	...
	public function music()
	{
		...
		if (preg_match('/^.+ghost-of-usenet\.org.+"(.+?)' . $this->e0 . '> >www\.SSL-News\.info<  - \(\d+\/\d+\) - \d+[.,]\d+ [kKmMgG][bB] yEnc$/', $this->subject, $match)) {
			return $match[1];
		} //"8 Wenn ich einmal gross bin .mp3" Koelschefetz postet.Die Filue -Immer Wigger  yEnc
		if (preg_match('/^"(.+?)' . $this->e0 . '.+yEnc$/', $this->subject, $match)) {
			return $match[1];
		...
		if (preg_match('/^\(\d+\/\d+\)[ -]{0,3}"(.+?)' . $this->e0 . '[- ]{0,3}\d+[.,]\d+ [kKmMgG][bB][- ]{0,3}.+yEnc$/', $this->subject, $match)) {
			return $match[1];
		}
		...
		if (preg_match('/^.+ \[\d+ of \d+\] "(.+?)' . $this->e1, $this->subject, $match)) {
			return $match[1];
		} //Wendy O Williams Bump n Grind - "Wendy.O.Williams.Bump.n.Grind.nfo"  yEnc
		if (preg_match('/^.+[ -]{0,3}"(.+?)' . $this->e1, $this->subject, $match)) {
			return $match[1];
	...
	public function music_mp3()
	{
		...
		if (preg_match('/^\[\d+\/\d+\][ -]{0,3}\(((Top)?\d+ )?(MP3 )?((Album|Single) )?Charts\)[ -]{0,4}"(.+?)' . $this->e0 . '  yEnc$/', $this->subject, $match)) {
			return $match[6];
		}
		...
		if (preg_match('/^[\(\[]\d+\/\d+[\]\)][ -]{0,3}\(MP3 Album\)[ -]{0,3}"(.+?)' . $this->e1, $this->subject, $match)) {
			return $match[1];
		...
		if (preg_match('/^.+usenet-piraten\.info.+[- ]{0,3}\[\d+\/\d+\][- ]{0,3}"(.+?)' . $this->e1, $this->subject, $match)) {
			return $match[1];
		} //[1/8] - "Nightwish - Storytime (Retail-Single 2011).sfv" -  ShapterV yEnc
		if (preg_match('/^\[\d+\/\d+\] - "(.+?)' . $this->e0 . '[- ]{0,4}ShapterV yEnc$/', $this->subject, $match)) {
			return $match[1];
		} //100 Rock - Vol.04<>usenet-piraten.info<>www.newsconnection.eu<> - (1/5)  "100 Rock - Vol.04.par2" - 101,65 MB - yEnc
		if (preg_match('/^.+usenet-piraten\.info.+newsconnection\.eu.+[- ]{0,3}\(\d+\/\d+\)[ -]{0,3}"(.+?)' . $this->e2, $this->subject, $match)) {
			return $match[1];
	...
	public function sound_mp3()
	{
		...
		if (preg_match('/^trtk\d+[ -]{0,3}\[\d+\/\d+\][ -]{0,3}"(.+?)' . $this->e1, $this->subject, $match)) {
			return $match[1];
		} //(????) [001/153] - "C4 House Party Horse Meat Disco Set 6.nfo" C4 House Party Horse Meat Disco Set 6 yEnc
		if (preg_match('/^\(\?+\) \[\d+\/\d+\][ -]{0,3}"(.+?)' . $this->e0 . '.+yEnc$/', $this->subject, $match)) {
			return $match[1];
		...
		if (preg_match('/^\[\d+\/\d+\][ -]{0,3}Top \d+ Album Charts \d+[ -]{0,3}"(.+?)' . $this->e0 . ' {1,4}yEnc$/i', $this->subject, $match)) {
			return $match[1];
		} //<>usenet-piraten.info<>partner<>ssl-news.info<> - [10/10] - "Overexposed (Deluxe Version).vol31+23.par2" yEnc
		if (preg_match('/^.+usenet-piraten\.info.+ - \[\d+\/\d+\][ -]{0,3}"(.+?)' . $this->e1, $this->subject, $match)) {
			return $match[1];
		...
		if (preg_match('/^\(\d+\/\d+\) - VERBAteamÂ present - "(.+?)' . $this->e2, $this->subject, $match)) {
			return $match[1];
		} //[1/1] - (Album Top 1000) - "Davis, Miles - Complete Live at the Plugged Nickel 1965.rar"  yEnc
		if (preg_match('/^\[\d+\/\d+\][ -]{0,3}\(Album Top \d+\)[ -]{0,3}"(.+?)' . $this->e0 . '  yEnc$/', $this->subject, $match)) {
			return $match[1];
	...
	public function sounds_lossless()
	{
		...
		if (preg_match('/^trtk\d+ - \[\d+\/\d+\] - "(.+?)' . $this->e1, $this->subject, $match)) {
			return $match[1];
	...
	public function sounds_mp3()
	{
		...
		if (preg_match('/^.+usenetrevolution.+Partner of ssl-news\.info.+\[\d+\/\d+\] - "(.+?)' . $this->e2, $this->subject, $match)) {
			return $match[1];
		} //<<<Old but Sold>>>  <<< >< >< "German Top 50 ODC - 12.08.2013.nfo" >< 02/33 (541,61 MB) >< 10,93 kB > yEnc
		if (preg_match('/^.+Old but Sold.+>< "(.+?)' . $this->e0 . ' >< \d+\/\d+ \(\d+[.,]\d+ [kKmMgG][bB]\).+ yEnc$/', $this->subject, $match)) {
			return $match[1];
		...
		if (preg_match('/^.+ghost-of-usenet.org[<>] [><]"(.+?)' . $this->e0 . '> >www\.SSL-News\.info<  - \(\d+\/\d+\)[- ]{0,3}\d+[.,]\d+ [kKmMgG][bB][- ]{0,3}yEnc$/', $this->subject, $match)) {
			return $match[1];
		} //BY REQ:........! - "Keith Whitley - All American Country - .par2" [06/22]  yEnc
		if (preg_match('/^BY REQ.+ - "(.+?)' . $this->e0 . ' \[\d+\/\d+\]  yEnc$/i', $this->subject, $match)) {
			return $match[1];
		} //Der etwas andere Mix - Wilde Herzenmix (auf wunsch) neu (by dem verrÃ¼ckten Lordi) (1/8) "Der etwas andere Mix - Wilde Herzenmix.par2" yEnc
		if (preg_match('/^Der etwas.+ \(\d+\/\d+\) "(.+?)' . $this->e0 . '.+yEnc$/i', $this->subject, $match)) {
			return $match[1];
		...
		if (preg_match('/^.+[\[\(]\d+\/\d+[\)\]][ -]{0,3}"(.+?)' . $this->e0 . '.+yEnc$/i', $this->subject, $match)) {
			return $match[1];
		} //"8 Wenn ich einmal gross bin .mp3" Koelschefetz postet.Die Filue -Immer Wigger  yEnc
		if (preg_match('/^"(.+?)' . $this->e0 . '.+yEnc$/', $this->subject, $match)) {
			return $match[1];
	...
	public function sounds_mp3_complete_cd()
	{
		...
		if (preg_match('/^\(\d+\/\d+\) "(.+?)' . $this->e0 . 'by_Funkeefly yEnc$/', $this->subject, $match)) {
			return $match[1];
		} //trtk09920 - [01/12] - "Guido Negraszus - Night Cafe Iii (Freedom Travellers) (2012)(320).par2" yEnc
		if (preg_match('/^trtk\d+[ -]{0,3}\[\d+\/\d+\][ -]{0,3}"(.+?)' . $this->e1, $this->subject, $match)) {
			return $match[1];
		...
		if (preg_match('/^>>> www.lords-of-usenet\.org <<< "(.+?)' . $this->e0 . '[-_ ]{0,3}\[\d+\/\d+\] - (\[)?\d+[.,]\d+ [kKmMgG][bB](\])? yEnc$/i', $this->subject, $match)) {
			return $match[1];
	...
	public function teevee()
	{
		...
		if (preg_match('/^\(Dgpc\) \[\d+\/\d+\] - "(.+?)' . $this->e1, $this->subject, $match)) {
			return $match[1];
		...
		if (preg_match('/^"([\w._-]{8,}.?)' . $this->e0 . '[- ]{0,3}\(\d+\/\d+\)[-_ ]{0,3}[\w. -]+[-_ ]{0,3}yEnc$/', $this->subject, $match)) {
			return $match[1];
		} //(????) [01/20] - "Boxing.Cinnamon vs El Perro.8th.March.2014.PDTV.x264-Sir.Paul.par2" Cinnamon vs El Perro  yEnc
		if (preg_match('/^\(\?+\)[-_ ]{0,3}\[\d+\/\d+\][-_ ]{0,3}"([\w._-]{8,}.?)' . $this->e0 . '[-_ ]{0,3}[\w. -]+[-_ ]{0,3}yEnc$/', $this->subject, $match)) {
			return $match[1];
		} //[REPOST]-"True.Blood.S06E01-E10.HDTV.x264.part094.rar" yEnc
		if (preg_match('/^\[[\w. -]+\][-_ ]{0,3}"([\w._-]{8,}.?)' . $this->e1, $this->subject, $match)) {
			return $match[1];
		...
		if (preg_match('/^\([\w. -]{8,}\)[-_ ]{0,3}\[\d+\/\d+\][-_ ]{0,3}""(.+?)' . $this->e0 . '"[-_ ]{0,3}[\w. -]+[-_ ]{0,3}yEnc$/', $this->subject, $match)) {
			return $match[1];
		...
		if (preg_match('/^[\w(). -]+[-_ ]{0,3}\[\d+\/\d+\][-_ ]{0,3}"([\w.()-]{8,}?)' . $this->e1, $this->subject, $match)) {
			return $match[1];
		...
		if (preg_match('/^\[[\w. -]{8,}\][-_ ]{0,3}\[\d+\/\d+\][-_ ]{0,3}"(.+?)' . $this->e2, $this->subject, $match)) {
			return $match[1];
		...
		if (preg_match('/^\[\]-\[FULL\]-\[a\.b\.teevee\]-\[[\w.-]+\][-_ ]{0,3}\[\d+\/\d+\][-_ ]{0,3}"(.+?)' . $this->e1, $this->subject, $match)) {
			return $match[1];
		...
		if (preg_match('/^\(\d+\/\d+\)[-_ ]{0,3}[\w. -]+[-_ ]{0,3}"(.+?)' . $this->e2, $this->subject, $match)) {
			return $match[1];
		...
		if (preg_match('/^\((.+)\) \[\d+\/\d+\][ _-]{0,3}("|#34;).+("|#34;) yEnc$/i', $this->subject, $match)) {
			return $match[1];
		...
		if (preg_match('/[Ss]\d+[Ee]\d+.+\[\d+ of \d+\] "(.+?)' . $this->e1, $this->subject, $match)) {
			return $match[1];
		} //Sam & Cat - S01E10 - BabysittingCommercial - "Sam & Cat - S01E10 - BabysittingCommercial.m4v" yEnc
		if (preg_match('/[Ss]\d+[Ee]\d+.+- "(.+?)' . $this->e1, $this->subject, $match)) {
			return $match[1];
		...
		if (preg_match('/^For teevee - "(.+?)' . $this->e0 . ' \[\d+\/\d+\][ _-]{0,3}yEnc$/', $this->subject, $match)) {
			return $match[1];
		} //"Mad.Men.S06E11.HDTV.x264-2HD.par2" yEnc
		if (preg_match('/^"(.+?)' . $this->e1, $this->subject, $match)) {
			return $match[1];
		...
		if (preg_match('/^\[[\w.-]+\][-_ ]{0,3}.+[-_ ]{0,3}\(\d+\/\d+\) "(.+?)' . $this->e2, $this->subject, $match)) {
			return $match[1];
		...
		if (preg_match('/^\[[\w. -]+\][-_ ]{0,3}.+[-_ ]{0,3}[\[\(]\d+\/\d+[\]\)][-_ ]{0,3}"(.+?)' . $this->e1, $this->subject, $match)) {
			return $match[1];
		} //Dave Gorman Modern Life is Goodish Series 1 - [114 of 126] - "Dave Gorman Modern Life is Goodish S01E06.mkv.008" yEnc
		if (preg_match('/^[\w -]+[-_ ]{0,3}[\[\(]\d+ of \d+[\]\)][-_ ]{0,3}"(.+?)' . $this->e1, $this->subject, $match)) {
			return $match[1];
		} //(1/9) "Tronens_Spil_S03E08.par2" yEnc
		if (preg_match('/^[\[\(]\d+\/\d+[\]\)][-_ ]{0,3}"([\w.()-]{8,}?)' . $this->e1, $this->subject, $match)) {
			return $match[1];
		...
		if (preg_match('/^\[ \d+[.,]\d+ [kKmMgG][bB] \][-_ ]{0,3}\[ \d+\/\d+ \][-_ ]{0,3}"([\w.()-]{8,}?)' . $this->e1, $this->subject, $match)) {
			return $match[1];
	...
	public function tv()
	{
		...
		if (preg_match('/^[\[\(]\w+[\]\)][- ]{0,3}\[\d+\/\d+\] - "([\w.()-]{8,}?)' . $this->e1, $this->subject, $match)) {
			return $match[1];
		...
		if (preg_match('/^"[\w ]+" yEnc[- ]{0,3}\[\d+\/\d+\][- ]{0,3}"([\w.()-]{8,}?)' . $this->e1, $this->subject, $match)) {
			return $match[1];
	...
	public function worms()
	{
		...
		if (preg_match('/^\(\d+\/\d+\) - Description - "(.+?)' . $this->e2, $this->subject, $match)) {
			return $match[1];
	...
	public function generic_town()
	{
		...
		if (preg_match('/^\(\d+\/\d+\) -<TOWN><www\.town\.ag >\s+<partner.+> - ("|#34;)(.+)(\.par2|-\.part\d+\.rar|\.nfo)("|#34;) - \d+[.,]\d+ [kKmMgG][bB]( -)? yEnc$/', $this->subject, $match)) {
			return $match[2];
		...
		if (preg_match('/^\(\d+\/\d+\).+?www\.town\.ag.+?sponsored by (www\.)?ssl-news\.info<+?.+? "(.+?)' . $this->e1, $this->subject, $match)) {
			return $match[2];
